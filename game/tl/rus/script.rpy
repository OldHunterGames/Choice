# TODO: Translation updated at 2023-11-27 15:59

# game/script.rpy:132
translate rus start_e5b27a15:

    # "I sauntered into the bustling nightclub, my eyes scanning the crowd for my next conquest."
    "Я вошел в шумный ночной клуб, сканируя глазами толпу в поисках моей сегодняшней цели."

# game/script.rpy:133
translate rus start_2a3fc6eb:

    # "The pulsing bassline of electronic music throbbed through the air as I made my way to the bar, ordering a drink with an easy smile on my lips."
    "Пульсирующая электронная музыка разгоняла моё сердцебиение, пока я пробирался к бару, заказывая напиток с легкой улыбкой на губах."

# game/script.rpy:134
translate rus start_8f26f005:

    # "I took a sip of my cocktail before casually glancing around again, searching for someone who caught my interest."
    "Я сделал глоток коктейля и снова огляделся по сторонам в поисках интересных, одиноко скучащих девушек."

# game/script.rpy:135
translate rus start_9dbe6d4e:

    # "Spotting her across the room - a young woman dancing alone near one of the VIP booths - my heart rate quickened in anticipation."
    "Мой взгляд остановился на ней - шикарной блондинке, танцующей в одиночестве возле одной из VIP-кабинок. Мое сердцебиение участилось в предвкушении."

# game/script.rpy:140
translate rus start_07c52191:

    # "She had curves that were impossible not to notice underneath her tight red dress."
    "Чувственные изгибы её тела, которые невозможно было не заметить под облегающим красным платьем, завораживали."

# game/script.rpy:141
translate rus start_a5ea051a:

    # "Girl's body moved instinctively to every beat of music; it was clear she wasn't just another pretty face but possessed an irresistible allure I couldn't miss out."
    "Тело девушки инстинктивно двигалось в такт музыке. За этим симпатичным личиком скрывалдось нечто большее - неотразимое обаяние, которое я не мог упустить."

# game/script.rpy:142
translate rus start_8a3d7f11:

    # "Without hesitation or second thought, I pushed through crowds towards her like a shark homing in on its prey."
    "Не раздумывая ни секунды, я ринулся сквозь толпу к ней, словно стрела, выпущенная точно в цель."

# game/script.rpy:143
translate rus start_143432ab:

    # "As I approached her from behind, I gently placed one hand on her shoulder causing goosebumps instantly forming along her skin at this unexpected touch from a stranger…"
    "Подойдя к ней сзади, я осторожно положил руку ей на плечо, отчего по её гладкой коже мгновенно побежали мурашки. Неожиданное прикосновение незнакомца заставило её обернуться..."

# game/script.rpy:148
translate rus start_d6940fa4:

    # pc "Hi, my name is Anton. (I said with a smile) I'm sorry if I scared you."
    pc "Здравствуйте, меня зовут Антон. (сказал я с улыбкой) Извините, если напугал."

# game/script.rpy:149
translate rus start_7cdfbf01:

    # pc "I just wanted to aquaint myself with the most beautiful girl in the club."
    pc "Просто никак не мог отказать себе в удовольствии познакомиться с самой красивой девушкой в клубе."

# game/script.rpy:150
translate rus start_65dfd03f:

    # g "I'm not really looking for anything serious right now"
    g "Знаешь, я не ищу сегодня ничего серьёзного."

# game/script.rpy:155
translate rus start_69175db0:

    # "A girl smiles to me, keeping her tone light and playful."
    "Пока она это произосила, на лице девушке расцвела игривая улыбка"

# game/script.rpy:156
translate rus start_2326755f:

    # "But there's a hint of curiosity in her eyes as we lock onto my."
    "В её глазах я читаю пробуждающийся интерес."

# game/script.rpy:157
translate rus start_b47fa099:

    # "I chuckled, leaning in closer to her ear so only she could hear."
    "Я вернул ей улыбку, наклонившись ближе к уху девушки, чтобы только она могла слышать, и прошептал игриво:"

# game/script.rpy:158
translate rus start_7e63d62a:

    # pc "Neither am I (whispered seductively)."
    pc "Знаешь, на самом деле, я тоже."

# game/script.rpy:159
translate rus start_036a9c2a:

    # pc "I just think you're absolutely stunning and want to show you a good time."
    pc "Просто меня стразили наповал твои движения и я решил что не помешает спросить, вдруг тебе будет приятна моя компания на вечер."

# game/script.rpy:160
translate rus start_614378b4:

    # "My hand slowly moved down her arm before entwining with hers on the bar counter, my fingers intertwined intimately."
    "Я медленно провёл пальцами по её руке, снизу вверх, и сделал другой рукой приглашающий жест в сторону бара."

# game/script.rpy:161
translate rus start_2d8d29d5:

    # pc "So... What brings such a gorgeous woman like yourself out alone tonight?"
    pc "Итак, что привело столь сногсшибательную женщину в этот клуб... в одиночестве?"

# game/script.rpy:162
translate rus start_d948aac2:

    # "Girl smiles back at me, feeling flattered by my attention."
    "Девушка вновь улыбается, показывая что ей нравится мой уверенный подход."

# game/script.rpy:163
translate rus start_a5e3d842:

    # "She likes the way I touch her and finds herself drawn to my confidence."
    "Моё смелое прикосновение и ненавязчивая твердость, страбатывают для неё как магнит."

# game/script.rpy:164
translate rus start_13238a2e:

    # g "Oh, you know… just looking for some fun"
    g "Ну знаешь... просто пришла развеяться после тяжелого рабочего дня."

# game/script.rpy:165
translate rus start_7a14ac58:

    # "She replied with a shrug that emphasized her breasts underneath the lacy black bra peeking out from beneath her dress."
    "Она ответила, пожав плечами, что подчеркнуло роскошную грудь под кружевным черным бюстгальтером, краешком выглядывающим из-под платья."

# game/script.rpy:166
translate rus start_4d8b13ee:

    # g "I'm Kristen by the way."
    g "И кстати, меня зовут Кристина."

# game/script.rpy:171
translate rus start_e5c494d7:

    # "She took another sip of her drink before adding:"
    "Она делает ещё один глоток из своего бокала, прежде чем продолжить:"

# game/script.rpy:172
translate rus start_95aad71d:

    # k "So what do you have in mind, macho-man?"
    k "Итак, с какой же целью вы столь нагло прервали мой танец, сударь?"

# game/script.rpy:173
translate rus start_ac5bb833:

    # "I nodded, grinning widely:"
    "Я киваю, показывая что оценил её шутку и готов её поддержать:"

# game/script.rpy:174
translate rus start_93c9d9fa:

    # pc "How about another round? On me."
    pc "О, позвольте мне загладить свою вину миледи, предложив вам напиток."

# game/script.rpy:175
translate rus start_95c11a06:

    # "I said as I turned to face the bartender, placing a hand on her lower back and guiding us both towards me."
    "Я повернулся к бармену и поманил его левой рукой, одновременно обвивая талию Кристины правой, чтобы притянуть девушку поближе к себе и подальше от шумной толпы."

# game/script.rpy:176
translate rus start_c20533c5:

    # "As we stepped closer to the bar, my other arm brushed against hers subtly but intentionally; feeling the softness of her skin beneath my fingertips sent shivers down my spine."
    "Пока мы ждём бармена, моя рука путешествует по её спине, поглаживая её кожу сквозь тонкую ткань облегающего платья."

# game/script.rpy:177
translate rus start_aa699678:

    # pc "What do you drink, gorgeous?"
    pc "Какой напиток усладит взыскательный вкус принцессы этой волшебной ночью?"

# game/script.rpy:182
translate rus start_15fc2f9d:

    # "Kristen smirked at my boldness, feeling a thrill run through her."
    "Кристина с улыбкой трясёт головой, намекая на то что наша шутка уже скатывается в кринж."

# game/script.rpy:183
translate rus start_fc4bbd0b:

    # "She liked this confident man who wasn't afraid to take charge."
    "Впрочем моя нахочивость и легкость ей явно по нраву."

# game/script.rpy:184
translate rus start_13cf97e7:

    # k "I'll have whatever you're having."
    k "Буду тоже что и ты."

# game/script.rpy:189
translate rus start_1d6dec1c:

    # "As I turned back towards her with our drinks in hand, Kristen couldn't help but notice how close we were standing; our bodies almost touching from hip-to-hip under the dim lighting of the bar area."
    "Когда я обернулся к ней с нашими напитками в руках, Кристен не могла не заметить, как близко мы стояли: наши тела почти соприкасались от бедра до бедра при тусклом освещении барной стойки."

# game/script.rpy:190
translate rus start_d0a8d105:

    # "The air between us seemed charged with electricity - anticipation and desire thick enough to cut with a knife…"
    "Воздух между нами, казалось, был заряжен электричеством - предвкушением и желанием, таким густым что хоть ложкой хлебай..."

# game/script.rpy:191
translate rus start_5617a64c:

    # pc "Here you go."
    pc "Прошу."

# game/script.rpy:192
translate rus start_62f742be:

    # "Handing her a fresh drink before taking one for myself. I clinked our glasses together playfully and took a sip."
    "Протягиваю ей напиток, а затем беру свой. Мы игриво чекнулись бокалами и сделали по глотку."

# game/script.rpy:193
translate rus start_350053a2:

    # pc "So tell me more about yourself"
    pc "Расскажешь мне немного о себе?"

# game/script.rpy:194
translate rus start_a8978bb2:

    # "I prompted it casually while keeping an eye on the dance floor nearby; itching to get closer to Kristen physically than we were at that moment."
    "Я начал непринужденную беседу, не сводя глаз с танцпола, расположенного неподалеку; мне не терпелось перейти к более тесному знакомству, но нужно было дать ей время привыкнуть."

# game/script.rpy:195
translate rus start_3d880a83:

    # pc "What do you like doing when you're not out here looking for fun?"
    pc "Чем занимаешься в свободное от развлечений время?"

# game/script.rpy:196
translate rus start_cc152cb9:

    # "Kristen took a sip of her drink, the cool liquid sliding down her throat as she thought about how to answer my question."
    "Кристина сделала глоток своего напитка, прохладная жидкость скользила по ее горлу, пока она думала, как ответить на мой вопрос."

# game/script.rpy:197
translate rus start_f865fa5a:

    # "She didn't want to come off too forward or desperate but also wanted me interested in continuing our time together after this bar stop…"
    "Она не хотела показаться слишком настойчивой или отчаянной, но в то же время хотела, чтобы я был заинтересован в продолжении нашего общения после остановки в баре..."

# game/script.rpy:198
translate rus start_1a9d5354:

    # k "I work for a pharmaceutical company"
    k "Работаю на одну крупную и совершенно бездушную фармацевтическую корпорацию."

# game/script.rpy:199
translate rus start_f7d309c7:

    # k "It pays well but can be quite boring at times."
    k "Платят хорошо, но гадюшник там невероятный."

# game/script.rpy:204
translate rus start_98c56e58:

    # "A small laugh escaped before adding with a wink:"
    "Она издает легкий смешок, прежде чем добавить:"

# game/script.rpy:205
translate rus start_e597a607:

    # k "But I make up for it by exploring my wild side when possible!"
    k "Приходится снимать стресс, выпуская по вечерам свою дикую пантеру, ррр!"

# game/script.rpy:210
translate rus start_74f69abc:

    # "I chuckled in response, nodding along with understanding. "
    "Я издаю легкий смешок, давая ей понять что оценил шутку."

# game/script.rpy:211
translate rus start_f9d76509:

    # pc "I can imagine"
    pc "Даже боюсь себе предтсавить!"

# game/script.rpy:212
translate rus start_a6f5f7d9:

    # "I said nonchalantly while taking another sip of my drink."
    "Делаю ещё один глоток."

# game/script.rpy:213
translate rus start_558885ad:

    # "In reality, I couldn't relate at all. My business was anything but boring for me!"
    "На самом деле мне её проблемы не близки. Я делаю деньги не выходя из дома и не имею дел ни с назойливыми колегами ни с тупыми клиентами."

# game/script.rpy:214
translate rus start_69dd4f0f:

    # "But it seemed like an interesting enough profession to keep the conversation going without revealing too much about myself just yet."
    "Однако надо проявить к ней живой интерес, чтобы поддержать разговор."

# game/script.rpy:215
translate rus start_1786c62c:

    # k "So what about you? (she asked curiously)"
    k "А ты сам чем занимаешься? (похоже ей действительно эти интересно)"

# game/script.rpy:216
translate rus start_d238b98a:

    # "Kristen tilting her head to the side as she watched me over the rim of her glass. She could feel my gaze on her body and wondered if I was thinking about more than just business right now…"
    "Кристен наклонила голову в сторону, наблюдая за мной через ободок своего бокала. Она чувствовала мой взгляд на своем теле и задавалась вопросом, не думаю ли я сейчас не только о бизнесе... "

# game/script.rpy:217
translate rus start_b1cc65d2:

    # "I smirked, taking another swig of my drink."
    "Я ухмыльнулся и сделал ещё один глоток, не спеша отвечать."

# game/script.rpy:218
translate rus start_e4c554ce:

    # pc "Oh, you know..."
    pc "Да так, знаешь..."

# game/script.rpy:219
translate rus start_53676a9a:

    # "I shrugged casually as if it were no big deal."
    "Я повел плечами, показывая что это меня мало заботит."

# game/script.rpy:220
translate rus start_ccf080ef:

    # pc "I have my hands in a few different things - investments mostly."
    pc "То одно то другое... портфельные инвестиции по большей части."

# game/script.rpy:221
translate rus start_c059abc6:

    # "I paused for dramatic effect before adding with a wink:"
    "Я сделал драматическую паузу и продолжил, многозначительно подмигнув:"

# game/script.rpy:222
translate rus start_27bd9568:

    # "But when I'm not making money hand over fist like this?"
    "Но когда мне надо избавитья от лишних денег и стресса..."

# game/script.rpy:223
translate rus start_01468ffb:

    # "I gestured vaguely around us towards the club and its revelers, implying that I enjoyed living life to the fullest just like Kristen did."
    "Я неопределенно указал в сторону людей отрывающихся на танцполе, подразумевая, что мне, как и Кристен, нравится жить полной жизнью."

# game/script.rpy:224
translate rus start_d7770b39:

    # "The music changed then; an upbeat pop song started playing on speakers nearby."
    "Словно повинуясь моему жесту, музыка сменилась на что-то бодро-танцевальное."

# game/script.rpy:225
translate rus start_594425e5:

    # k "That's great actually. But I'm here not for a small talk. Let's dance!"
    k "Круто. Потому что я здесь не для светских бесед. Бежим танцевать!"

# game/script.rpy:232
translate rus start_026ce9f1:

    # "Kristen grabbed my hand and pulled me onto the dance floor, her body moving instinctively to the beat as she pressed against my."
    "Кристина хватает меня за руку и с удивительной силой тащит в сторону танцпола. Прижавшись ко мне спиной она начинает танцевать."

# game/script.rpy:233
translate rus start_8f609274:

    # "She could feel my hardness through our clothes; it sent a shiver of anticipation down her spine."
    "Прикосвноения её спортивно попки сквозь брюки заставляют мой член напрячься и пропускают по позвоночнику дрожь предвкушения."

# game/script.rpy:234
translate rus start_57772597:

    # "As we swayed together underneath strobing lights, she whispered into my ear:"
    "Пока мы двигались вместе под светом прожекторов, она прошептала мне на ухо:"

# game/script.rpy:235
translate rus start_db327b64:

    # k "Enjoy the show."
    k "А теперь гляди внимательно..."

# game/script.rpy:236
translate rus start_441599fe:

    # "Her breath was hot against my skin, teasing me with promises unspoken but clear in her eyes…"
    "Ее дыхание горячо касалось моей кожи, дразня меня невысказанными, но ясными обещаниями в ее глазах..."

# game/script.rpy:237
translate rus start_e0627988:

    # "Kristen's hips swayed hypnotically to the music, her body moving in perfect rhythm with every beat."
    "Бедра Кристен гипнотически покачивались в такт музыке, тело двигалось в идеальном ритме с каждым ударом басов."

# game/script.rpy:238
translate rus start_943c8d31:

    # "She gyrated and twirled around I, making sure I had an unobstructed view of her curves as she danced closer and closer until we were almost touching."
    "Она кружилась и кружилась вокруг меня, обеспечивая беспрепятственный обзор своих изгибов, танцуя все ближе и ближе, пока мы почти не соприкоснулись."

# game/script.rpy:239
translate rus start_fddd7b86:

    # "Her breath came faster now; anticipation mixed with desire coursed through her veins."
    "Ее дыхание участилось, предвкушение, смешанное с желанием, струилось по ее венам."

# game/script.rpy:240
translate rus start_1fbf4d07:

    # "Her gaze locked onto my - a silent invitation for more than just this dance…"
    "Ее взгляд остановился на мне - молчаливое приглашение к чему-то большему, чем просто танец..."

# game/script.rpy:241
translate rus start_79acda5c:

    # "I couldn't take my eyes off of Kristen as she danced seductively for me."
    "Я не мог оторвать глаз от Кристины, когда она соблазнительно танцевала для меня."

# game/script.rpy:253
translate rus start_71741f0c:

    # "The sight was intoxicating, and I felt himself growing harder with every move she made."
    "Зрелище было упоительным, и я чувствовал, как с каждым ее движением наливается кровью мой член."

# game/script.rpy:254
translate rus start_472f7779:

    # "I stepped closer to her until our bodies were pressed together; the heat between us palpable even through our clothes."
    "Я придвинулся к ней ближе, и наши тела оказались прижаты друг к другу; жар между нами ощущался даже сквозь одежду."

# game/script.rpy:255
translate rus start_7c44dac0:

    # pc "You are exquisite!"
    pc "Ты потрясающая!"

# game/script.rpy:256
translate rus start_31ff0192:

    # "I murmured against her ear before trailing kisses down her neck, my hands roaming over her body possessively underneath the thin fabric separating us."
    "Я прошептал ей на ухо и стал осыпать поцелуями ее шею, мои руки властно блуждали по ее телу под тонкой тканью, разделяющей нас."

# game/script.rpy:257
translate rus start_cfd382f1:

    # pc "Let me show you how much I appreciate your performance."
    pc "Позволь показать, насколько меня вдохновил твой танец."

# game/script.rpy:258
translate rus start_1c6edd2b:

    # "Kristen shivered at my touch, her heart racing in anticipation. She leaned into me, moaning softly as I kissed along her neck and shoulders."
    "Кристен задрожала от моего прикосновения, а ее сердце забилось в предвкушении. Она прильнула ко мне и тихо застонала, когда я поцеловал ее шею и плечи."

# game/script.rpy:259
translate rus start_e1140b11:

    # k "Mmm… that sounds tempting."
    k "Ммм... звучит заманчиво."

# game/script.rpy:260
translate rus start_a99f822f:

    # "She is pulling away slightly to look up at me with a mischievous glint in her eye."
    "Она слегка отстраняется и смотрит на меня с озорным блеском в глазах."

# game/script.rpy:261
translate rus start_691c6989:

    # k "But I have a better idea."
    k "Но у меня есть идея получше."

# game/script.rpy:262
translate rus start_3f443951:

    # "With that, she grabbed my hand and led us both towards the exit."
    "С этими словами она взяла меня за руку и повела нас обоих к выходу."

# game/script.rpy:266
translate rus start_b3bbbd27:

    # "As we stepped out into the cool night air, Kristen glanced around to make sure no one was watching before pulling I towards a dark alleyway between two buildings."
    "Когда мы вышли на прохладный ночной воздух, Кристина оглянулась, чтобы убедиться, что за нами никто не наблюдает, и потянула меня в темный переулок между двумя зданиями."

# game/script.rpy:267
translate rus start_f99e5756:

    # "Her heart pounded in her chest with excitement and nerves as she pressed herself against me once more - this time feeling my arousal clearly through our clothes."
    "Ее сердце колотилось в груди от волнения и нервного напряжения, когда она снова прижалась ко мне - четко ощущая мое возбуждение сквозь одежду."

# game/script.rpy:268
translate rus start_7fd58ee0:

    # k "I have an empty apartment in a downtown."
    k "У меня есть пустая квартира минутах в десяти отсюда, если на машине."

# game/script.rpy:269
translate rus start_a4b5a941:

    # "She whispered breathlessly into my ear, eyes sparkling with mischief."
    "Она прошептала мне на ухо с придыханием, глаза блестели озорством:"

# game/script.rpy:270
translate rus start_e38356c0:

    # k "Want to come up for some real fun?"
    k "Может продолжим наше знакомство у меня?"

# game/script.rpy:271
translate rus start_bf1a2d2c:

    # "I smirked, my hands sliding up her back to cup her ass gently."
    "Я ухмыльнулся, мои руки скользнули по ее спине и нежно обхватили ее попку."

# game/script.rpy:272
translate rus start_de74eea1:

    # pc "I think that's an excellent idea."
    pc "Какая замечательная идея. Идём!"

# game/script.rpy:273
translate rus start_83268e00:

    # "I purred in response, pulling away just enough to look into those sparkling eyes of hers."
    "Я мурлыкал в ответ, отстраняясь настолько, чтобы заглянуть в ее искрящиеся глаза."

# game/script.rpy:274
translate rus start_09245ca0:

    # k "Where is your car?"
    k "Где твоя машина?"

# game/script.rpy:275
translate rus start_2dd2e3b8:

    # "Kristen was definitely interested in seeing what I was driving."
    "Ей похоже очень интересно, на чем я езжу."

# game/script.rpy:276
translate rus start_a44d73b4:

    # "I chuckled softly, leading her over to my sleek black sports car parked nearby."
    "Я загадочно ухмыльнулся и повел ее к своему элегантному черному спортивному автомобилю, припаркованному неподалеку."

# game/script.rpy:277
translate rus start_a11169a5:

    # pc "Right here."
    pc "Карета подана, сударыня."

# game/script.rpy:278
translate rus start_45c05b1f:

    # "I unlocked the doors for us both before climbing into the driver's seat."
    "Я разблокировал двери для нас обоих и сел на водительское сиденье. Впрочем, руль это чистая формальность в автомобиле с автопилотом четвертого поколения."

# game/script.rpy:279
translate rus start_62ad4d46:

    # "The interior of the vehicle was just as luxurious as its exterior - leather seats and smooth surfaces everywhere you looked."
    "Интерьер автомобиля был таким же роскошным, как и его внешний вид - кожаные сиденья, прозрачная крыша и экраны повсюду, куда бы вы ни посмотрели."

# game/script.rpy:280
translate rus start_c39d65a3:

    # "I turned towards Kristen, one eyebrow raised in question; she seemed eager but hesitant at the same time…"
    "Я повернулся к Кристен, вопросительно приподняв одну бровь; она выглядела жаждущей, но в то же время нерешительной..."

# game/script.rpy:281
translate rus start_47d5ae90:

    # "Kristen nodded, climbing into the passenger seat and buckling her seatbelt."
    "Кристен кивнула, забралась на пассажирское сиденье и пристегнула ремень безопасности."

# game/script.rpy:282
translate rus start_048adb7f:

    # "Let's go!"
    "Погнали!"

# game/script.rpy:283
translate rus start_63963288:

    # "She said it with determination, already imagining what we could get up to in my car…"
    "Она сказала это решительно, уже представляя, что мы можем устроить в моей машине, пока автопилот рулит."

# game/script.rpy:288
translate rus start_214a9dad:

    # "The drive to Kristen's apartment was filled with tension and anticipation."
    "Но я не из тех кто торопит события. Поездка в квартиру Кристен была наполнена напряжением и предвкушением."

# game/script.rpy:289
translate rus start_ee7fb4f4:

    # "I could feel her eyes on me as I navigated through the city streets, my hands tight around the steering wheel from both excitement and nerves."
    "Я чувствовал на себе ее взгляд, когда ехал по городским улицам, и мои руки блуждали по её бедрам подрагивающим от волнения и возбуждения."

# game/script.rpy:290
translate rus start_9913ea8e:

    # "She leaned back in her seat, playing coyly with her hair or crossing/uncrossing her legs while occasionally glancing out of the window - teasing me without touching."
    "Она откинулась на спинку сиденья, жеманно играя волосами, скрещивая вновь раздвигая ноги, изредка поглядывая в окно - дразнила меня, не прикасаясь."

# game/script.rpy:291
translate rus start_84a90acd:

    # "Finally pulling up outside an unassuming building near downtown, I turned off the engine but made no move to get out just yet"
    "Остановившись у неприметного здания недалеко от центра города, я заглушил двигатель, но выходить из машины пока не стал."

# game/script.rpy:292
translate rus start_75f8eeee:

    # "I was looking at this woman who had captivated me so completely since our first encounter at the bar…"
    "Я смотрел на эту женщину, которая так пленила меня с момента нашей первой встречи в баре..."

# game/script.rpy:293
translate rus start_d4b8b152:

    # "Kristen's heart raced as we pulled up outside her apartment building. This was it - the moment of truth."
    "Сердце Кристины бешено колотилось, когда мы остановились возле ее дома. Вот он - момент истины."

# game/script.rpy:294
translate rus start_43438814:

    # "She turned towards me, biting her bottom lip nervously but determined not to show any hesitation or fear."
    "Она повернулась ко мне, нервно покусывая нижнюю губу, но твердо решив не показывать ни колебаний, ни страха."

# game/script.rpy:298
translate rus start_50896b69:

    # k "Well? Aren't you coming in?"
    k "Ну что? Ты идёшь?"

# game/script.rpy:299
translate rus start_65c90d58:

    # "I smirked, stepping out of the car and rounding it to her side."
    "Я ухмыльнулся, выходя из машины и огибая ее со стороны."

# game/script.rpy:300
translate rus start_a248948f:

    # pc "Of course."
    pc "Разумеется."

# game/script.rpy:301
translate rus start_81b81fc3:

    # "I bow to her slightly before opening a door for her with a flourish."
    "Я слегка поклонился ей, после чего галантно открыл перед ней дверь."

# game/script.rpy:302
translate rus start_e53568c0:

    # "My hand brushed against hers as she exited; our skin connecting once more underneath the cool night air."
    "Когда она выходила, моя рука коснулась ее руки теплой на фоне ночной прохлады."

# game/script.rpy:303
translate rus start_c83cf296:

    # "We walked up together towards Kristen's apartment building entrance, my arm wrapping around her waist possessively while hers found its way onto my shoulder - both of us lost in each other already…"
    "Мы вместе шли к подъезду дома, где жила Кристины, моя рука собственнически обхватывала ее талию, а ее - мое плечо - мы оба уже потерялись друг в друге..."

# game/script.rpy:304
translate rus start_6820ecdb:

    # "We stepped into the elevator together, pressing against each other as it ascended to her floor."
    "Мы вместе вошли в лифт, прижавшись друг к другу, пока он поднимался на ее этаж."

# game/script.rpy:306
translate rus start_c66caadd:

    # "The doors slid open and Kristen led me down a short hallway towards her unit before unlocking it with shaking hands."
    "Двери открылись, и Кристен провела меня по короткому коридору к своей двери, а затем отперла её."

# game/script.rpy:310
translate rus start_4d3e59b8:

    # "Inside was an immaculate one-bedroom space - minimalistic but comfortable looking; perfect for our needs right now…"
    "Внутри оказалось безупречное помещение с одной спальней - минималистичное, но уютное, идеально подходящее для наших нужд..."

# game/script.rpy:311
translate rus start_951b2409:

    # "She turned around to face me."
    "Она повернулась ко мне, глядя прямо в мои глаза."

# game/script.rpy:316
translate rus start_d01335e8:

    # k "Here we are, make yourself comfortable."
    k "Вот мы и пришли. Чувствуй себя как дома."

# game/script.rpy:317
translate rus start_39e0b82b:

    # pc "What are we gonna do? (asked in a sultry tone)"
    pc "Чем займёмся?"

# game/script.rpy:322
translate rus start_36762b2f:

    # "Kristen pouted her lips in slight annoyance."
    "Кристен надула свои пухлые губки демонстрируя легкое раздражение."

# game/script.rpy:323
translate rus start_6fd46a44:

    # k "Hey, macho, you're the one who made a move on me at the bar. So you tell me. What do you want? Make a choice!"
    k "Эй, мачо, это же ты сделал подкатил ко мне в баре, вот ты мне и скажи. Чего ты хочешь? Сделай выбор!"

# game/script.rpy:336
translate rus make_love_c484aaa4:

    # "I smiled, stepping closer to her."
    "Я улыбнулся и подошёл к ней ближе."

# game/script.rpy:337
translate rus make_love_f12c52eb:

    # pc "I think I know exactly what I want"
    pc "Думаю ты догадываешься, чего я хочу."

# game/script.rpy:342
translate rus make_love_0af9e75a:

    # "I make my voice low and seductive, reaching out to run a finger down the side of her cheek before leaning in for a slow kiss that left goosebumps on her skin."
    "Мурлыча низким и соблазнительным голосом я протянул руку, чтобы провести пальцем по ее щеке, а затем наклонился для медленного поцелуя, от которого по ее коже побежали мурашки."

# game/script.rpy:343
translate rus make_love_1476df14:

    # pc "You're so damn sexy."
    pc "Ты такая горячая."

# game/script.rpy:344
translate rus make_love_8283aa7d:

    # "I whispered against her lips as we parted once more."
    "Я шепчу, легонько прикасаясь к её губам своими."

# game/script.rpy:345
translate rus make_love_21d021cf:

    # "My hands trailed over every inch of exposed flesh we could find; tracing along the line of her collarbone, teasingly brushing across nipples through fabric, sliding lower still until cupping full hips possessively."
    "Мои руки скользят вдоль линию её обнаженных ключиц, дразняще провели по соскам через ткань и спустились ниже, пока не обхватили полные бедра."

# game/script.rpy:346
translate rus make_love_36ebecfe:

    # pc "You know, out there on the dance floor. you were moving so sexy. I couldn't stop watching."
    pc "Знаешь, там, на танцполе. Ты двигалась так сексуально. Я просто не мог оторваться."

# game/script.rpy:347
translate rus make_love_175acea6:

    # pc "The only thing missing was that you didn't take your clothes off in the process."
    pc "Единственное, чего не хватало, так это того, чтобы при ты при этом раздевалась."

# game/script.rpy:348
translate rus make_love_410aaa46:

    # k "I guess that would have been reckless in front of so many people, huh?"
    k "Наверное, это было бы безрассудно на глазах у стольких людей, да?"

# game/script.rpy:349
translate rus make_love_3048827d:

    # "Kristen laughts."
    "Кристина смеётся."

# game/script.rpy:350
translate rus make_love_7355f567:

    # pc "Yeah… but we're alone now."
    pc "Да уж... но зато теперь, мы одни. И..."

# game/script.rpy:351
translate rus make_love_69f56d97:

    # "I sit down on a chair, making myself comfortable."
    "Я отошёл от неё и сел на стул, рапсоложившись поудобнее."

# game/script.rpy:352
translate rus make_love_925d5653:

    # pc "And I want to see you stripping, baby."
    pc "... я хочу увидеть стриптиз в твоём исполнении, детка."

# game/script.rpy:353
translate rus make_love_719ec771:

    # "Kristen blushed, feeling her heart race at my demand."
    "Кристен покраснела, почувствовав, как заколотилось ее сердце от моего требования."

# game/script.rpy:354
translate rus make_love_b591d5bb:

    # k "Look's like it's your lucky day, Anton!"
    k "Знаешь, сегодня тебе везет как никогда, Антон!"

# game/script.rpy:355
translate rus make_love_f0de617d:

    # "She turned on the music and let it fill the room as she started to dance sensually - hips swaying in sync with the beat."
    "Она включила музыку и позволила ей заполнить комнату, начав чувственно танцевать - бедра покачивались в такт ритму."

# game/script.rpy:356
translate rus make_love_31994809:

    # "Slowly, she unzipped her red dress and slipped it off her shoulders; revealing more of her curves as it fell to floor in a seductive pool around heels."
    "Медленно расстегнув молнию на красном платье, она спустила его с плеч, обнажив все свои изгибы. Платье падает на пол напоминая красную лужицу вокруг каблуков."

# game/script.rpy:361
translate rus make_love_b00d6c26:

    # "Her black lacy underwear was all that remained now, barely hiding anything from view…"
    "Оставалось только черное кружевное белье, которое почти ничего не скрывало от глаз..."

# game/script.rpy:362
translate rus make_love_b0e8513f:

    # "With each step and dip, she teased me further."
    "С каждым шагом и погружением она все больше дразнила меня."

# game/script.rpy:363
translate rus make_love_2c526119:

    # "Her body glistening under the dim light as her breasts bounced enticingly with every move."
    "Ее тело блестело в тусклом свете, а груди соблазнительно подпрыгивали при каждом движении."

# game/script.rpy:364
translate rus make_love_2682e898:

    # "She faced me again, eyes locked on my - daring me to take what I wanted…"
    "Кристина снова стояла передо мной, не сводя с меня глаз - смеялась взять то, что я хотел..."

# game/script.rpy:365
translate rus make_love_ef0671a2:

    # k "Like what you see?"
    k "Как тебе зрелище?"

# game/script.rpy:366
translate rus make_love_5a93e894:

    # "Her voice is low and sultry."
    "Её голос звучит низко и соблазительно."

# game/script.rpy:367
translate rus make_love_7dd1eda6:

    # pc "I want to see you all!"
    pc "Хочу увидеть всё!"

# game/script.rpy:368
translate rus make_love_1442d5d7:

    # "Without hesitation, Kristen reached behind her and unclasped the back of her bra."
    "Плавными движением, Кристина потянулась за спину и расстегнула застежку бюстгальтера."

# game/script.rpy:369
translate rus make_love_1e5c006c:

    # "It fell away, revealing her perfect breasts to my eager gaze."
    "Он упал, открыв моему жадному взгляду ее идеальную грудь."

# game/script.rpy:370
translate rus make_love_a3aeab54:

    # "She ran a finger over one nipple before cupping both in her hands and pushing us together for my viewing pleasure…"
    "Она провела пальцем по одному соску, затем взяла оба в руки и прижала нас друг к другу, чтобы я мог полюбоваться..."

# game/script.rpy:371
translate rus make_love_76f77c05:

    # "With a wicked grin, Kristen hooked her thumbs into the waistband of her black lacy underwear and slowly slid us down over her hips."
    "С лукавой улыбкой Кристен зацепила большими пальцами краешек своих кружевных трусиков и медленно спустила их вниз по бедрам."

# game/script.rpy:372
translate rus make_love_55631dbd:

    # "She stepped out of them, leaving herself completely naked before my hungry eyes…"
    "Сбросив их она остаётся совершенно обнаженной перед моими голодными глазами..."

# game/script.rpy:383
translate rus make_love_99fd43cd:

    # k "Are you satisfied now?"
    k "Теперь ты доволен?"

# game/script.rpy:384
translate rus make_love_785b1ea8:

    # "Kristen is tilting her head to one side as she watched my awe in the sight of her bare body."
    "Кристен наклонила голову в одну сторону, наблюдая за моим благоговейным взглядом ласкающим её обнаженное тело."

# game/script.rpy:385
translate rus make_love_b9054224:

    # "Her heart raced with excitement at my reaction; each breath sending shivers down her spine…"
    "Моё сердце бешено колотилось от волнения, каждый вдох вызывал мурашки по позвоночнику..."

# game/script.rpy:386
translate rus make_love_be667702:

    # "I stood up, my cock hard as a rock in my pants."
    "Я встал, мой член в штанах был тверд как камень."

# game/script.rpy:387
translate rus make_love_97264e5b:

    # pc "Mmmm, you're perfect, baby."
    pc "О да, ты само совершентво."

# game/script.rpy:388
translate rus make_love_1d9a29a6:

    # "I stepped closer to her and ran a hand down her stomach, tracing the curve of her hip before circling around to cup her perfect breast."
    "Я подошел к ней ближе и провел рукой по ее животу вниз, прослеживая изгиб бедра, а затем поместил в ладонь ее идеальную грудь."

# game/script.rpy:389
translate rus make_love_29444ad8:

    # "My thumb brushed over her nipple, causing it to harden even more under my touch."
    "Мой большой палец провел по ее соску, заставив его еще больше затвердеть под моим прикосновением."

# game/script.rpy:390
translate rus make_love_7237ecba:

    # pc "Come here,"
    pc "Иди ко мне."

# game/script.rpy:391
translate rus make_love_a697858c:

    # "I growled softly against her ear before picking her up and carrying us both towards the coach."
    "Я тихонько зарычал ей в ухо, а затем подхватил ее на руки и к дивану."

# game/script.rpy:422
translate rus blowjob_00401aec:

    # "I took a step closer to her, my eyes burning with desire as I ran my hands up and down her naked body."
    "Я держал её в своих объятьях и мои глаза горели желанием, когда я провел руками по ее обнаженному телу."

# game/script.rpy:423
translate rus blowjob_0a8b99b2:

    # pc "You're even more beautiful without your clothes on."
    pc "Без одежды ты даже более прекрасна."

# game/script.rpy:424
translate rus blowjob_fd844183:

    # "Kristen felt her cheeks flush at my compliment, her nipples hardening under my touch."
    "Кристен почувствовала, как ее щеки раскраснелись от моего комплимента, а соски затвердели от моего прикосновения."

# game/script.rpy:425
translate rus blowjob_a29176d1:

    # "She looked up at me with shiny gray eyes, a small smile playing on her lips as she waited for my next move."
    "Она смотрела на меня блестящими серыми глазами, на ее губах играла соблазнительная улыбка в ожидании моего следующего шага."

# game/script.rpy:426
translate rus blowjob_15a8e5a0:

    # "I pulled away slightly, giving Kristen a sly grin. I turned back around slowly, giving Kristen time to admire my ass which flexed enticingly beneath those tight jeans"
    "Я слегка отстранился, одарив Кристину лукавой ухмылкой. Я медленно повернулся назад, давая той время полюбоваться своей задницей, под обтягивающими джинсами."

# game/script.rpy:427
translate rus blowjob_0dbd6fdd:

    # "With one swift movement, I removed them along with boxers revealing a thick cock already half-hard from anticipation alone"
    "Одним движением я снял их вместе с трусами, обнажив толстый член, уже полутвердый от одного только предвкушения."

# game/script.rpy:428
translate rus blowjob_5d0f3030:

    # "Kristen gulped audibly when she saw how big it was; bigger than most men she had been with previously..."
    "Кристен громко ахнула, увидев, какой он большой; больше, чем у большинства мужчин, с которыми она была раньше..."

# game/script.rpy:429
translate rus blowjob_14c6af87:

    # "... but not intimidating enough to make her hesitate anymore because now all thoughts were focused solely on pleasuring me."
    "...но не настолько пугающий, чтобы заставить ее колебаться, потому что теперь все мысли были сосредоточены исключительно на том, чтобы доставить мне удовольствие."

# game/script.rpy:430
translate rus blowjob_4443692c:

    # "As I turned back around, Kristen couldn't help but let out a soft moan of desire."
    "Кристина не удержалась и издала тихий стон желания."

# game/script.rpy:431
translate rus blowjob_627cdebd:

    # "She had never seen such a formidable cock before - thick and veiny, pulsing with need for her mouth."
    "Она еще никогда не видела такого грозного члена - толстого, с прожилками, пульсирующего от желания наполнить собой её в ротик."

# game/script.rpy:434
translate rus blowjob_8928921b:

    # "Without another word, she dropped to her knees in front of me, taking my throbbing member into her hands and gently stroking it up and down the shaft as she leaned forward to take the head between her lips."
    "Не говоря ни слова, она опустилась передо мной на колени, взяла в руки мой пульсирующий член и, наклонившись вперед, обнимая головку теплыми губаими, нежно поглаживая стержень вверх и вниз."

# game/script.rpy:435
translate rus blowjob_3babdab3:

    # "Her tongue flicked over the sensitive tip, tasting the pre-cum that had already gathered there."
    "Она провела языком по чувствительному кончику, пробуя на вкус выступившую там капельку предварительной смазки."

# game/script.rpy:436
translate rus blowjob_85ddb219:

    # "She closed her eyes tightly, savoring the taste as she began to bob her head up and down on my shaft, sucking deeper each time."
    "Она закрыла глаза, концентрируясь на процессе, и стала двигать головой вверх-вниз по моему стволу, с каждым разом насаживаясь все глубже."

# game/script.rpy:437
translate rus blowjob_1aabb95f:

    # "Her slurping sounds echoed through the room while she worked her magic, using every ounce of skill she possessed to make this blowjob unforgettable."
    "Хлюпающие звуки эхом разносились по комнате, пока она творила свое волшебство, используя свое изрядное мастерство, чтобы сделать этот минет незабываемым."

# game/script.rpy:438
translate rus blowjob_822d3f8c:

    # pc "Oh, fuck... You're natural!"
    pc "Ох, блядь... а ты талантливая!"

# game/script.rpy:439
translate rus blowjob_5c38ab34:

    # "I groaned loudly, my hands finding their way into her hair as I gripped tightly."
    "Я громко застонал, запустив руки в ее волосы и крепко сжав их."

# game/script.rpy:440
translate rus blowjob_18d33364:

    # "My hips thrusting forward involuntarily against her skilled mouth. This was better than anything I could have imagined!"
    "Мои бедра непроизвольно подались вперед навстречу ее искусному рту. Это было лучше, чем все, что я мог себе представить!"

# game/script.rpy:441
translate rus blowjob_fa1be145:

    # "As Kristen continued her blowjob, she could feel my cock growing harder and heavier in her mouth."
    "Продолжая делать минет, Кристен чувствовала, как мой член становится все тверже и тяжелее в ее рту."

# game/script.rpy:442
translate rus blowjob_aedb68af:

    # "She took more of it each time, swallowing around him when he reached the back of her throat."
    "С каждым разом она брала все больше и больше, заглатывая его, пока он полностью не поник в глубину ее горла."

# game/script.rpy:443
translate rus blowjob_16c5a524:

    # "Her eyes remained closed as she focused on pleasing me completely - tasting every inch, feeling my moans vibrate against her face."
    "Ее глаза оставались закрытыми, поскольку она полностью сосредоточилась на том, чтобы доставить мне удовольствие - попробовать каждый сантиметр."

# game/script.rpy:444
translate rus blowjob_1d4eac7b:

    # pc "You're amazing, baby!"
    pc "Это невероятно, детка!"

# game/script.rpy:445
translate rus blowjob_e1d6d793:

    # "I groaned out between gasps for air."
    "Мой голос срывается от удовольствия"

# game/script.rpy:446
translate rus blowjob_f6a457d8:

    # pc "I've never had a woman suck like this before."
    pc "Ещё никто прежде мне так не отсасывал."

# game/script.rpy:447
translate rus blowjob_751a830f:

    # "My words only fueled Kristen's determination to make this night one I would never forget!"
    "Мои слова только подстегнули решимость Кристен сделать эту ночь незабываемой для меня!"

# game/script.rpy:448
translate rus blowjob_7ebcd2ef:

    # "With renewed vigor, she increased the speed and pressure of her bobbing head."
    "С новой силой она насаживается своим горлом на мой пульсирующий член."

# game/script.rpy:449
translate rus blowjob_c7e811a6:

    # "Kristen felt a rush of pride hearing those words from me, but she knew there was more to come."
    "Кристен почувствовала прилив гордости, услышав эти слова от меня, но она знала, что впереди еще много интересного."

# game/script.rpy:450
translate rus blowjob_bea36ec6:

    # "She slowed her pace slightly, teasing me with gentle sucks and licks around the head before taking me back into her mouth again."
    "Она слегка замедлила темп, дразня меня нежными посасываниями и облизываниями головки, прежде чем снова заглотить мой хуй глубоко в рот."

# game/script.rpy:451
translate rus blowjob_e8702b36:

    # "Her hands wandered down my thighs, tracing patterns on my skin as she waited for my next command."
    "Ее руки блуждали по моим бедрам, вырисовывая узоры на моей коже, пока она ждала моей следующей команды."

# game/script.rpy:459
translate rus sex_4ae496a9:

    # "I swiftly moved forward, pressing my lean yet ripped body against her soft curves."
    "Я стремительно подался вперед, прижимаясь своим напряженным телом к ее мягким изгибам."

# game/script.rpy:460
translate rus sex_dfca5e73:

    # "My hands glide over every inch of her supple skin, trailing up towards her firm large breasts that jiggle enticingly under my touch."
    "Мои руки скользят по каждому сантиметру ее нежной кожи, поднимаясь к упругим большим грудям, которые соблазнительно покачиваются под моими прикосновениями."

# game/script.rpy:461
translate rus sex_66c80789:

    # "With a deft movement, I couped one in each hand and gently squeeze them before lowering my head to capture a throbbing nipple between my lips."
    "Уверенным движением я положил по ладони на каждую грудь и нежно сжал их, а затем опустил голову, чтобы захватить пульсирующий сосок между губами."

# game/script.rpy:462
translate rus sex_c1941162:

    # "Kristen moans softly as she throws back her head onto the rack of their shoulders, allowing me greater access while arching her spine towards me for more contact."
    "Кристен тихонько застонала, откинув голову на плечи, облегчая мне доступ и одновременно выгибая позвоночник навстречу мне для большего контакта."

# game/script.rpy:463
translate rus sex_d33e36e1:

    # "I suck greedily at it with practiced skill even as the other hand slides downward along her taut stomach and lower until I reach the wetness between her thighs..."
    "Я жадно ласкаю её сосок ртом, в то время как другая рука скользит вниз по ее упругому животу и ниже, пока не достигает влажного пространства между бедрами..."

# game/script.rpy:464
translate rus sex_09b9fa49:

    # "...finding it already eagerly awaiting me!"
    "...обнаруживая что она уже потекла от возбуждения!"

# game/script.rpy:465
translate rus sex_02bce89a:

    # "As we neck deep into the intimate dance of desire and lust on the couch beneath us in silence broken only by our heavy breathing and occasional gasps of pleasure."
    "Мы погружаемся в интимный танец желания и похоти на диване под нами в тишине, нарушаемой только тяжелым дыханием и периодическими вздохами удовольствия."

# game/script.rpy:466
translate rus sex_4d2a7fdb:

    # "I slowly push two fingers deep into tender folds soaking wet with desire - searching out Kristen's sweet spot effortlessly despite being unfamiliar territory thus far"
    "Я медленно ввожу два пальца в нежные складочки, мокрые от желания, и без труда нащупываю сладкую точку Кристен, с радостью исследуя неизведынные территории."

# game/script.rpy:467
translate rus sex_38b94612:

    # "A shudder runs through both our bodies when I finally find what I seek"
    "Дрожь пробегает по обоим нашим телам, когда мои настойчивые пальцы находят свою цель."

# game/script.rpy:468
translate rus sex_4200b40d:

    # "Thrusting those digits rhythmically inside making sure not miss any part of the hidden treasure trove which causes goosebumps all over both flesh & bones alike!"
    "Ритмично двигая пальцами, я запускаю волну по телу Кристен волну наслаждения прокатывающуюся мурашками по всему её телу."

# game/script.rpy:469
translate rus sex_ab8fec6e:

    # "While fucking herself on those invading digits relentlessly now Kristen is frustrated by lack proximity alone wanting full cock instead"
    "Страстно насаживаясь на вторгающиеся в некё поальцы Кристен жаждет большего. Она хочет почувствовать в себе мой член."

# game/script.rpy:470
translate rus sex_25f4c9a3:

    # "Now craving release more than anything else world could"
    "Наша жажда наслаждения и сладкой развязки поглощает собой всё."

# game/script.rpy:471
translate rus sex_f1cc39ba:

    # k "Oh, Anton! Fuck me! (moans impatiently) I want you cock in my slit… please fuck me!"
    k "О, Антон! Выеби меня! (нетерпеливый стон) Я хочу твой член в моей щелке... пожалуйста, трахни меня!"

# game/script.rpy:472
translate rus sex_dd4774ec:

    # "Feeling the heat and need emanating from Kristen, I pull away from her plump lips reluctantly."
    "Чувствуя жар и потребность, исходящие от Кристен, я неохотно отстраняюсь от ее пухлых губ."

# game/script.rpy:473
translate rus sex_e2d3b7b9:

    # "My strong hands grip her thighs as I slowly stand up to tower over her aching form."
    "Мои сильные руки обхватывают ее бедра, когда я медленно встаю и возвышаюсь над ее изгибающейся фигурой."

# game/script.rpy:474
translate rus sex_07640c91:

    # "With a wicked smirk, I watch as she spreads herself wide in anticipation of my entry."
    "С лукавой ухмылкой я наблюдаю, как она широко раздвигает ноги в ожидании моего проникновения."

# game/script.rpy:476
translate rus sex_f99617d5:

    # "I waste no time stepping between those legs and pressing the crown of my cock against her entrance"
    "Не теряя времени, я встаю между ее ног и прижимаю головку члена к ее входу."

# game/script.rpy:477
translate rus sex_57dc9e40:

    # "Teasingly grazing it back and forth before finally thrusting my dick deep within with one powerful motion that causes both our bodies to shudder"
    "Дразняще поглаживая его взад-вперед, я ввожу свой член глубоко внутрь одним мощным движением, от которого содрогаются оба наших тела."

# game/script.rpy:478
translate rus sex_3139a861:

    # "As our hips grind together instinctively seeking friction, I take control by wrapping an arm around her waist pulling me closer while using the other hand for leverage"
    "Пока наши бедра трутся друг о друга, я беру обхватываю ее талию и притягиваю к себе."

# game/script.rpy:479
translate rus sex_19a942c1:

    # "This angle allows for deeper penetration each time we connect intimately."
    "Такой угол позволяет добиться более глубокого проникновения каждый раз, когда я двигаю бёдрами вперёд."

# game/script.rpy:480
translate rus sex_55911bd3:

    # "The sound of slapping flesh echoes throughout the room along with whimpers from Kristen's lips parted in a plea for more - begging me not to stop tmy pleasure!"
    "Шлепки соприкасающейся плоти эхом разносится по комнате вместе с хныканьем Кристен, губы которой раздвинуты в мольбе о большем - умоляя меня не прекращать удовольствие!"

# game/script.rpy:481
translate rus sex_932c309d:

    # k "Ahh… fuck! (Kristen moans) I feel it… so deep inside me… Please, don't stop. Fuck me harder!"
    k "Ааа... блядь! Я чувствую его... так глубоко внутри меня... Пожалуйста, не останавливайся. Трахай меня сильнее!"

# game/script.rpy:482
translate rus sex_cc2fff7b:

    # "I grin at Kristen's begging, my deep black eyes reflecting the passion within me as I start to pump my hips in a steady rhythm that matches our entwined breaths."
    "Я ухмыляюсь в ответ на мольбы Кристен, мои глубокие черные глаза отражают страсть внутри меня, когда я начинаю двигать бедрами в устойчивом ритме, который совпадает с нашим слитным дыханием."

# game/script.rpy:483
translate rus sex_e6a24025:

    # "Each thrust drives my dick deeper inside before pulling out just enough to tease and taunt before plunging back in again."
    "С каждым толчком мой член проникает все глубже, а затем выходит ровно настолько, чтобы подразнить, после чего снова погружается внутрь."

# game/script.rpy:484
translate rus sex_e23ead6b:

    # "My strong yet gentle grip on her waist holds her close as we sway together like dancers lost in an intimate waltz under the haze of lustful desire."
    "Моя сильная, но нежная хватка на ее талии удерживает Кристину рядом, и мы качаемся вместе, как танцоры, заблудившиеся в интимном вальсе под дымкой похотливого желания."

# game/script.rpy:485
translate rus sex_3c56801b:

    # "Kristen arches her back off the couch in pure ecstasy feeling every inch of my manhood slide against sensitive walls within"
    "Кристен откидывается на спинку дивана в чистом экстазе, чувствуя, как каждый дюйм моего мужского достоинства скользит по чувствительным стенкам внутри её лона."

# game/script.rpy:486
translate rus sex_0879218c:

    # "Clawing at anything she can reach for purchase to keep herself grounded during these moments of primal bliss."
    "Она вцепляется ногтями во все, до чего может дотянуться, чтобы выдержать эти минуты первобытного блаженства."

# game/script.rpy:487
translate rus sex_24bcbf1b:

    # "Fucking harder and faster until sweat beads form along their skin from exertion alone!"
    "Мы двигаемся все сильнее и быстрее, от напряжения с нас течёт пот."

# game/script.rpy:488
translate rus sex_e872c478:

    # "Her breasts bounce enticingly against my chest with each movement adding another layer of heightened stimulation between us both"
    "Ее сиьки соблазнительно задевают мою грудь сосками, каждое движение добавляет еще один слой усиленного возбуждения между нами обоими."

# game/script.rpy:489
translate rus sex_0731e231:

    # "Moans and groans echo off every surface signaling how close we are getting not only physically but also emotionally!"
    "Стоны и вздохи отражаются от каждой поверхности, сигнализируя о том, как близко мы находимся не только физически, но и эмоционально."

# game/script.rpy:496
translate rus anal_1257adf5:

    # "With a smooth confidence, I take the lead by wrapping my strong arms around her waist and pulling her body close to my own."
    "С ровной уверенностью я беру инициативу в свои руки, обхватывая ее талию сильными руками и притягивая тело Кристины к себе."

# game/script.rpy:497
translate rus anal_bc92e61f:

    # "I wmyper softly into her ear"
    "Я тихо шепчу ей прямо в ухо:"

# game/script.rpy:498
translate rus anal_5a01c821:

    # pc "You really are stunningly beautiful"
    pc "Сейчас ты ещё красивее чем когда я увидел тебя."

# game/script.rpy:499
translate rus anal_7ed3fdb0:

    # "My voice sending shivers down Kristen's spine."
    "Я вижу как от звука моего голоса по телу Кристины пробегает сладкая дрожь."

# game/script.rpy:500
translate rus anal_6ec18212:

    # "She melts into my embrace, unable to resist the power of my charm as I nuzzle against her neck while gently sucking on her earlobe."
    "Она тает в моих объятиях, не в силах сопротивляться силе моего обаяния, когда я прижимаюсь к ее шее, нежно посасывая мочку её уха."

# game/script.rpy:501
translate rus anal_aba9b8c5:

    # "Our tongues dance together in an erotic dance that leaves us both panting for more intimacy."
    "Наши языки танцуют вместе в эротическом танце, который заставляет нас обоих жаждать еще большей близости."

# game/script.rpy:502
translate rus anal_9f8da3ab:

    # "Ever so gently, I guide Kristen towards the coach as I position her body to bend over it."
    "Нежно но настойчиво я подвожу Кристен к дивану, располагая ее тело так, чтобы она могла глубоко нагнуться опираясь на него."

# game/script.rpy:503
translate rus anal_cacc9f1c:

    # "My intentions are clear - I gonna fuck her in the ass."
    "Она понимает что я хочу сдеалть - выебать её в задницу."

# game/script.rpy:504
translate rus anal_f5c1ad54:

    # "Taking my time, I fetch a small bottle of lubricant and slowly uncaps it before approaching her sensitive anus."
    "Не торопясь, я достаю маленькую бутылочку с лубрикантом и медленно откупориваю ее, прежде чем приблизиться к ее чувствительному анусу."

# game/script.rpy:505
translate rus anal_9e28b5e7:

    # "Unsure but intrigued by my actions, Kristen watches nervously as I dabble some lube onto my fingers and tease her opening with them."
    "Неуверенная, но заинтригованная моими действиями, Кристен нервно наблюдает за тем, как я наношу немного смазки на свои пальцы и дразню ими ее дырочку."

# game/script.rpy:506
translate rus anal_dd972810:

    # "As anticipation builds within me."
    "Во мне нарастает нетерпение."

# game/script.rpy:507
translate rus anal_e90511ed:

    # "Bracing herself for any discomfort that may come, Kristen feels my lubricated finger penetrate her anal ring slowly."
    "Приготовившись к возможному дискомфорту, Кристина чувствует, как мой смазанный палец медленно проникает в сквозь её испуганно сжатый сфинтктер."

# game/script.rpy:508
translate rus anal_f53c2570:

    # "The tightness around it causes a slight twinge of pain; she winces slightly but bears through it out of trust for me."
    "Тугое сжатие вокруг пальца вызывает легкую боль; Кристина слегка морщится, но терпит из доверия ко мне."

# game/script.rpy:509
translate rus anal_214c6208:

    # "As I introduce another digit and begins to stretch her opening, she finds herself pleading softly"
    "Когда я ввожу еще один палец и начинаю растягивать ее попку, она нежно умоляет"

# game/script.rpy:510
translate rus anal_6e1c9417:

    # k "I… haven't done thus before. Be gentle please."
    k "Я... никогда прежде так не пробовала. Пожалуйста нежнее..."

# game/script.rpy:511
translate rus anal_9165782f:

    # "Her voice quivers with fear mixed with arousal as she continues to beg me to cease the unfamiliar act."
    "Ее голос дрожит от страха, смешанного с возбуждением, когда она продолжает умолять меня прекратить непривычный для нё процесс."

# game/script.rpy:512
translate rus anal_70f1b4d6:

    # "I kiss her shoulder reassuringly while wmypering soothing assurances in her ear"
    "Я ободряюще целую ее плечо, шепча успокоительные слова ей на ушко:"

# game/script.rpy:513
translate rus anal_85e45d5c:

    # pc "I won't hurt you. I promise."
    pc "Тебе понравится. Я обещаю."

# game/script.rpy:514
translate rus anal_9d21b854:

    # "My other hand slides down between our bodies rubbing against the hot wetness between her legs, teasing both of their budding desires."
    "Моя вторая рука скользит вниз между нашими телами, потираясь о горячую влагу между ее ног, отвлекая и дразня её."

# game/script.rpy:515
translate rus anal_dcf2ec8e:

    # "I gently thrust my fingers deeper into her anus until they reach deep within where we meet resistance from the sensitive walls surrounding my knuckles before pulling back..."
    "Я осторожно ввожу пальцы глубже в ее попку, пока они не достигают глубины, где встречают сопротивление чувствительных стенок, окружающих мои костяшки пальцев, после чего отступаю назад..."

# game/script.rpy:516
translate rus anal_a06620db:

    # "...and pushing again!"
    "...и проталкиваю вновь!"

# game/script.rpy:517
translate rus anal_fe6de943:

    # "Despite her unease, Kristen feels a building desire within herself that matches my."
    "Несмотря на беспокойство, Кристен чувствует в себе нарастающее желание, ловя эмоционально мою страсть."

# game/script.rpy:518
translate rus anal_19f97b6a:

    # "She can't help but moan as I continues to penetrate her tight hole with each thrust of my fingers."
    "Она не может удержаться от стона, когда я продолжаю проникать в ее тугую дырочку с каждым движением пальцев."

# game/script.rpy:519
translate rus anal_cc2425bc:

    # "The sensation brings an entirely new level of pleasure she hadn't experienced before - one that ignites a fire within her core."
    "Ощущение приносит совершенно новый уровень удовольствия, которого она не испытывала раньше, и разжигает огонь в ее сердце."

# game/script.rpy:520
translate rus anal_dbd0da67:

    # "Her hips buck against me, craving more contact as we both struggle for release."
    "Ее бедра упираются в мои, жаждут большего контакта, пока мы оба боремся за разрядку."

# game/script.rpy:521
translate rus anal_1ae929dd:

    # "I kiss along her spine, breathing heavily into the crook of her neck; unable to contain my excitement for claiming the beautiful woman's ass for myself."
    "Я целую ее вдоль позвоночника, тяжело дыша в изгиб ее шеи, не в силах сдержать свое возбуждение от желания заполучить эту прекрасную женскую попку себе."

# game/script.rpy:524
translate rus anal_d69153c2:

    # "I pull back slightly and position myself at her entrance before slowly pushing into the depths with just the head of my cock"
    "Я слегка отстраняюсь и становлюсь у ее входа, а затем медленно ввожу в глубину только головку члена."

# game/script.rpy:525
translate rus anal_c4e9c717:

    # "Pausing momentarily due to resistance from her anal muscles squeezing around me tightly like velvet gloves around steel rod."
    "На мгновение я приостановился из-за сопротивления ее анальных мышц, плотно обхвативших меня, словно бархатные перчатки сжтимающие стальной стержень."

# game/script.rpy:526
translate rus anal_2cecbd2b:

    # "With determination burning through any last vestiges of hesitation, I push deeper until I feel our hips meet flush together joining them in one sinful motion"
    "С решимостью, сжигающей последние остатки нерешительности, я проникаю глубже, пока не чувствую, как наши бедра встречаются вровень, соединяясь в едином греховном движении."

# game/script.rpy:527
translate rus anal_6dc7ea5f:

    # "As I begin to thrust gently inside of Kristen's now stretched and swollen anus, I feels a rush of pleasure unlike any other."
    "Когда я начинаю осторожно проникать внутрь растянутого ануса Кристины, я чувствую прилив удовольствия, не похожего ни на какое другое."

# game/script.rpy:528
translate rus anal_4d6bbbeb:

    # "It's clear she's tight yet accommodating - her walls clenching around my cock tightly, driving me wild with desire."
    "Видно, что она тугая, но податливая - ее стенки плотно сжимаются вокруг моего члена, доводя меня до дикого желания."

# game/script.rpy:529
translate rus anal_a582c056:

    # "I start off slowly but find it difficult to control my urge for harder pounding due to the intensity of need coursing through my veins."
    "Я начинаю медленно, но мне трудно сдерживать желание сильнее колотиться из-за интенсивности потребности, текущей по моим венам."

# game/script.rpy:530
translate rus anal_ce96c6b8:

    # "The sight alone sends chills down my spine as I watch my cock disappear into her rectum; each successive thrust pushing deeper into her virgin depths."
    "От одного этого зрелища у меня по позвоночнику бегут мурашки, когда я смотрю, как мой член исчезает в ее прямой кишке, с каждым толчком проникая все глубже в ее девственные глубины."

# game/script.rpy:531
translate rus anal_8c892991:

    # k "Ah… It's a strange feeling… (moans) Don't push it so much… it hurts!"
    k "А... Странное ощущение... Не дави так сильно... Это больно!"

# game/script.rpy:539
translate rus cum_face_3e3ea487:

    # "I gripped her hair tighter, pulling her off just enough to look into those shiny gray eyes."
    "Я крепче схватил ее волосы, отстраняясь, чтобы заглянуть в эти блестящие серые глаза."

# game/script.rpy:540
translate rus cum_face_002de19a:

    # pc "You're killing me. (growled playfully) I want you to take all of this cock."
    pc "Я хочу, чтобы ты взяла весь этот член."

# game/script.rpy:541
translate rus cum_face_96d88359:

    # pc "(paused for a moment before continuing softly) Show me what you can do with your throat."
    pc "Покажи мне, насколько у тебя натренированное горло."

# game/script.rpy:542
translate rus cum_face_ed89978b:

    # "Kristen couldn't hide the excitement in her voice as she nodded eagerly."
    "Кристен скрывает волнение но решительно кивает мне."

# game/script.rpy:543
translate rus cum_face_21a90660:

    # "She opened wide, taking my dick back into her mouth until its entire length was buried deep within it once again"
    "Она широко раскрыла рот и снова взяла мой член в рот, пока он не оказался в нем на всю длину."

# game/script.rpy:544
translate rus cum_face_86009145:

    # "Moaning around my shaft as if it were filling up every orifice instead."
    "Стоны Кристины отдаются вибрацией вдоль моего мясистого стержня."

# game/script.rpy:545
translate rus cum_face_330a6fc6:

    # "Her hands moved up my torso now, fisting in my shirt and pulling me closer still as she began bobbing faster than ever before!"
    "Ее руки двинулись вверх по моему торсу, вцепились в мою рубашку и притянули меня еще ближе, а она начала двигаться быстрее, чем когда-либо прежде!"

# game/script.rpy:546
translate rus cum_face_35328241:

    # "I couldn't hold back any longer."
    "Я больше не в силах сдерживаться."

# game/script.rpy:547
translate rus cum_face_0677d1ec:

    # "I began thrusting my hips forward, fucking her mouth with a fervor that left no doubt about who was in control now!"
    "Я начал двигать бедрами вперед, трахая ее рот с пылом, который не оставлял сомнений в том, кто сейчас контролирует ситуацию!"

# game/script.rpy:548
translate rus cum_face_acd2e0ed:

    # "Every time I pulled and pushed in and out, she took more of my lenght eagerly, sucking and licking every inch as it slid past her lips."
    "Каждый раз, когда я втягивал и выталкивал его, она с жадностью принимала все большую длину моего члена, прижимая язык к каждому сантиметру проскальзывающего в ее ротик члена."

# game/script.rpy:549
translate rus cum_face_a729ac90:

    # "Her cheeks hollowed out from the force of each powerful stroke, but she welcomed the sensation - this was exactly what she had been waiting for!"
    "Ее щеки стягиваются от силы каждого мощного толчка, но она наслаждается ощущениями - это было именно то, чего она ждала!"

# game/script.rpy:550
translate rus cum_face_8cf84a5d:

    # pc "Fuck yeah, (groaned, feeling himself getting closer to the edge) Get it all the way in, like a good girl."
    pc "Да, блядь, (простонал, чувствуя, что приближается к краю) принимай его до конца, как хорошая девочка."

# game/script.rpy:551
translate rus cum_face_ca8f35a8:

    # "My hands gripped her head tightly, holding her in place as I used her body like a personal pleasure device."
    "Мои руки крепко обхватили ее голову, удерживая ее на месте, пока я использовал голову Кристины как персональный мастурбатор."

# game/script.rpy:552
translate rus cum_face_db8b63d2:

    # pc "I'm close, (I warned her hoarsely between pants for air) Don't stop now!"
    pc "Я уже близко, - хрипло предупредил я ее между потугами вдохнуть воздух. - Не останавливайся!"

# game/script.rpy:553
translate rus cum_face_6ce5d999:

    # "Kristen felt her throat stretching to its limits, the gag reflex threatening to take over as my cock hit the back of her mouth with each powerful thrust."
    "Кристина чувствовала, как ее горло растягивается до предела, а рвотный рефлекс грозил взять верх, когда мой член с каждым мощным толчком врезался в ее рот."

# game/script.rpy:554
translate rus cum_face_04504116:

    # "She tried desperately not to choke on it, taking short breaths in between sucks and licks when she could manage it."
    "Она отчаянно пыталась не захлебнуться, делая короткие вдохи между сосанием и облизыванием, когда ей это удавалось."

# game/script.rpy:555
translate rus cum_face_390d8f2a:

    # "Her eyes were watering now from the effort but never once did they leave his gaze - she wouldn't give up until I came!"
    "Ее глаза слезились от напряжения, но она ни на секунду не отрывала своего взгляда - она не сдастся, пока я не кончу!"

# game/script.rpy:556
translate rus cum_face_391f14ca:

    # pc "Fuck... I can feel it. (muttered through clenched teeth) Suck it, suck it, suuuck!"
    pc "Черт... Я чувствую это. Соси, соси, соси, сосииии!"

# game/script.rpy:557
translate rus cum_face_bef1efec:

    # "I pulled out just enough for her to get a breath before slamming back into that wet warmth again, driving a cock deeper than ever before!"
    "Я отстранился ровно настолько, чтобы она смогла перевести дух, а затем снова погрузился в это влажное тепло, вгоняя член глубже, чем когда-либо прежде!"

# game/script.rpy:558
translate rus cum_face_4f07a12a:

    # "Kristen groaned around him, feeling every vein bulge against her tongue and cheeks as she worked harder than ever to please me completely!"
    "Кристина застонала, чувствуя, как каждая жилка вздувается на ее языке и щеках, ведь она старалась изо всех сил, чтобы полностью удовлетворить меня!"

# game/script.rpy:559
translate rus cum_face_f55d31e8:

    # "My hips bucked wildly, my cock throbbing in her mouth as I neared orgasm."
    "Мои бедра бешено вращались, член пульсировал в ее рту, приближаясь к оргазму."

# game/script.rpy:560
translate rus cum_face_1bf11697:

    # "Kristen could feel it building inside of her - the warmth and pressure growing stronger with each passing second."
    "Кристина чувствовала, как нарастает внутри нее тепло и давление, усиливаясь с каждой секундой."

# game/script.rpy:561
translate rus cum_face_a5af2946:

    # "She increased the speed of her bobbing head, taking more of a dick into that tight throat until she felt him pulse violently against her tongue!"
    "Она увеличивала скорость движений, полностью вбирая мой пульсирующий хуй в своё тугое горло, пока не почувствовала, как он яростно пульсирует на ее языке!"

# game/script.rpy:562
translate rus cum_face_3ec3a3f4:

    # pc "Oh fuck!"
    pc "Да, блядь!"

# game/script.rpy:563
translate rus cum_face_32ff843f:

    # "I cry out loudly, shooting ropes of hot cum down her throat as she swallowed every drop greedily!"
    "Я громко кричу, выплескивая потоки горячей спермы ей в горло, пока она жадно глотает каждую каплю!"

# game/script.rpy:564
translate rus cum_face_c138a47e:

    # "Kristen pulled back slowly from my softening member, licking around the crown to clean off any remaining traces of cum"
    "Кристина медленно отстранилась от моего размягчающегося члена, облизывая его вокруг головки, чтобы смыть все оставшиеся следы спермы."

# game/script.rpy:565
translate rus cum_face_47e1235d:

    # "She is looking up at me with pride etched across those shiny gray eyes."
    "Она смотрит на меня с гордостью в блестящих серых глазах."

# game/script.rpy:574
translate rus cum_belly_9458dc94:

    # "I lean down capturing one plump nipple between my teeth as moan vibrates through it."
    "Я наклоняюсь, захватывая один пухлый сосок между зубами, и стон вибрирует в нем."

# game/script.rpy:575
translate rus cum_belly_8f809a72:

    # "Sending shivers through entire body line causing waves after wave crash over us both sweeping away any lingering thoughts or worries"
    "По всему телу бегут мурашки, заставляя волны за волнами обрушиваться на нас обоих, унося прочь любые затянувшиеся мысли и тревоги."

# game/script.rpy:576
translate rus cum_belly_ceae3f45:

    # "Leaving only raw need slow building towards imminent climax."
    "Оставляя только животную потребность, медленно приближающуюся к неизбежной кульминации."

# game/script.rpy:577
translate rus cum_belly_138a09a7:

    # "Feeling the increasing rhythm of my movements, Kristen realizes that I am close to orgasm."
    "Чувствуя нарастающий ритм моих движений, Кристина понимает, что я близок к оргазму."

# game/script.rpy:578
translate rus cum_belly_afd1d74f:

    # k "Please don't come yet! (pleads desperately) I'm so close. Ah… ah! Harder. Please! Just don't stop!!! Don't stop!"
    k "Пожалуйста, не кончай пока! (умоляюще) Я так близко. Ах... ах! Сильнее. Пожалуйста! Только не останавливайся!!! Не останавливайся!"

# game/script.rpy:579
translate rus cum_belly_0c6fd208:

    # "I feel the raging waves of pleasure building up within me, my muscles tensing as I fights to hold back my release."
    "Я чувствую, как внутри меня нарастают волны наслаждения, мышцы напрягаются, пытаясь сдержать разрядку."

# game/script.rpy:580
translate rus cum_belly_00b0dee8:

    # "The sensation of Kristen's warmth enveloping me and her moans echoing through the room are enough to drive me wild with need."
    "Ощущения тепла Кристины, окутывающего меня, и ее стонов, разносящихся по комнате, достаточно, чтобы довести меня до дикой потребности."

# game/script.rpy:581
translate rus cum_belly_428fa195:

    # "With a growl that mirrors my impending orgasm, I increases the intensity of my thrusts without mercy - pounding into her over and over again"
    "С рыком, отражающим мой приближающийся оргазм, я увеличиваю интенсивность своих толчков, беспощадно вбиваясь в нее снова и снова."

# game/script.rpy:582
translate rus cum_belly_ab6d12b4:

    # "I move like a man possessed by desire itself."
    "Я двигаюсь, как человек, одержимый самим желанием."

# game/script.rpy:583
translate rus cum_belly_1366c28e:

    # "Kristen gasps for air between each cry of pleasure, feeling every inch of my powerful frame against hers as we dance tmy erotic waltz towards ecstasy."
    "Кристина задыхается от наслаждения, ощущая каждый сантиметр моего мощного тела на своем, пока мы танцуем эротический вальс навстречу экстазу."

# game/script.rpy:584
translate rus cum_belly_d9f110a9:

    # "Her tight channel grips at me eagerly while she arches her back begging for more; closer to that elusive peak we both crave so dearly!"
    "Ее тугой любовный канал жадно обнимает мой член, когда девушка выгибает спину, умоляя о большем, ближе к тому неуловимому пику, которого мы оба так жаждем!"

# game/script.rpy:585
translate rus cum_belly_97a1f192:

    # "She can feel it building deep inside; becoming unbearable almost… but not quite yet ready just when I give an audible groan signaling imminent climax!"
    "Она чувствует, как он нарастает глубоко внутри, становится почти невыносимым... но еще не совсем готов, как раз когда я издаю звучный стон, сигнализирующий о приближающейся кульминации!"

# game/script.rpy:586
translate rus cum_belly_6983c983:

    # pc "Ahhhh... yes, yes, yessss..."
    pc "Аххх.... да, Да! ДАААА!!!"

# game/script.rpy:587
translate rus cum_belly_81c9d5fd:

    # "In response, Kristen's nails dig into my shoulders drawing blood as she pulls herself closer on reflex alone trying anything possible keep from being left behind in momentary bliss!"
    "В ответ на это ногти Кристен впиваются в мои плечи до крови, и она рефлекторно притягивает меня ближе, пытаясь сделать все возможное, чтобы не остаться позади в миг блаженства!"

# game/script.rpy:588
translate rus cum_belly_5adc933a:

    # "The world around us melts away into pure sensation creating an indescribable connection so intense neither could resist much longer now…"
    "Мир вокруг нас растворяется в чистых ощущениях, создавая неописуемую связь, настолько сильную, что ни один из нас не может долго сопротивляться..."

# game/script.rpy:589
translate rus cum_belly_3b3f2158:

    # "My hips buck wildly, pouring every ounce of my energy into each thrust as I nears my climax."
    "Мои бедра бешено вращаются, вливая каждую унцию энергии в каждый толчок, пока я приближаюсь к кульминации."

# game/script.rpy:590
translate rus cum_belly_05b2fc88:

    # "I grit my teeth, feeling the familiar burn building deep within me."
    "Я стиснул зубы, чувствуя, как внутри меня разгорается знакомое жжение."

# game/script.rpy:591
translate rus cum_belly_b558fa1d:

    # "Kristen feels it too—the telltale signs that I is about to spill my seed inside her."
    "Кристен тоже чувствует это - признаки того, что я вот-вот пролью свое семя прямо в нее."

# game/script.rpy:592
translate rus cum_belly_7f3a6f5f:

    # "Her body tightens around me in anticipation, welcoming the oncoming release with open arms."
    "Ее тело сжимается вокруг меня в предвкушении, встречая предстоящую разрядку с распростертыми объятиями."

# game/script.rpy:593
translate rus cum_belly_cb4da7f5:

    # "With a primal roar, I finally unleash my hot cum within her trembling womb."
    "С первобытным рыком я наконец выплеснул свою горячую сперму в ее трепещущее лоно."

# game/script.rpy:594
translate rus cum_belly_86ba6300:

    # "My powerful member throbs and pulses as a sticky white liquid fills every nook and cranny."
    "Мой мощный член пульсирует и бьется, а липкая белая жидкость заполняет каждый уголок внутри неё."

# game/script.rpy:595
translate rus cum_belly_dd04a6e6:

    # "She clamps down on me instinctively at first before succumbing to overwhelming pleasure herself."
    "Сначала она инстинктивно прижимается ко мне, а затем и сама поддается непреодолимому наслаждению."

# game/script.rpy:596
translate rus cum_belly_ee0f687e:

    # k "Oh, I'm cuming! I'm cuming!!!"
    k "О, я кончаю! Я кончаю!!!"

# game/script.rpy:597
translate rus cum_belly_4e3db1b4:

    # "She screams out in ecstasy as wave after wave washes over us both like an ocean tide crashing onto a rocky shore - our passion so intense it feels almost tangible!"
    "Она кричит в экстазе, а волна за волной накрывает нас обоих, как океанский прилив, разбивающийся о скалистый берег, - наша страсть настолько сильна, что кажется почти осязаемой!"

# game/script.rpy:598
translate rus cum_belly_0d89f0d3:

    # "Our bodies become one glowing entity bathed in sweat and satisfaction under the torrent of raw emotion while their hearts race against each other's chests"
    "Наши тела превращаются в единое светящееся целое, купающееся в поту и удовлетворении под потоком необработанных эмоций, а их сердца бьются о грудь друг друга."

# game/script.rpy:599
translate rus cum_belly_f3797064:

    # "I pull out of Kristen with a soft groan, my cock still twitching from the intense release."
    "Я выхожу из Кристины с тихим стоном, мой член все еще дергается от интенсивной разрядки."

# game/script.rpy:600
translate rus cum_belly_113b8610:

    # "A thick string of cum oozes out from her entrance, leaving a wet trail behind as it leaks onto the couch."
    "Густая струйка спермы вытекает из ее входа, оставляя за собой мокрый след и стекая на диван."

# game/script.rpy:601
translate rus cum_belly_f65e9492:

    # "I watch in fascination as she gathers some on her fingers and brings it to her lips, savoring the taste with closed eyes and parted mouth"
    "Я завороженно наблюдаю, как она собирает немного на пальцы и подносит к губам, наслаждаясь вкусом с закрытыми глазами и приоткрытым ртом."

# game/script.rpy:602
translate rus cum_belly_3b40b0eb:

    # "This is revealing just how much pleasure she's found in this encounter."
    "Это показывает, какое удовольствие она получила от этой встречи."

# game/script.rpy:603
translate rus cum_belly_f88ba902:

    # "With shaky hands we both sit up straight trying to catch our breath; leaning against one another for support."
    "С трясущимися руками мы оба сидим прямо, пытаясь перевести дыхание и опираясь друг на друга для поддержки."

# game/script.rpy:604
translate rus cum_belly_971d7eac:

    # "Kristen runs gentle fingers along my abs feeling every ripple beneath before finally letting go allowing me space explore surroundings somewhat numbly"
    "Кристина проводит нежными пальцами по моему прессу, ощущая каждую его пульсацию, и наконец отпускает, позволяя мне несколько оцепенело исследовать окружающее пространство."

# game/script.rpy:613
translate rus cum_back_951d9a23:

    # "I take Kristen from behind slowly, allowing her body to adjust to the foreign sensation of being penetrated anally."
    "Я медленно беру Кристен сзади, позволяя ее телу привыкнуть к чужому ощущению анального проникновения."

# game/script.rpy:614
translate rus cum_back_d43923f4:

    # "Kissing her neck gently as a sign of reassurance that I am here for her pleasure too."
    "Я нежно целую ее шею в знак того что наше удовольствие будет взаимным."

# game/script.rpy:615
translate rus cum_back_e86a026b:

    # "My thrusts become shallower but more rhythmic, letting her relax into the feeling of fullness inside her tight hole."
    "Мои толчки становятся более слабыми, но ритмичными, позволяя ей расслабиться, ощущая как мой член заполняет её попку."

# game/script.rpy:616
translate rus cum_back_ad49d633:

    # "As she begins to loosen up around me, I can feel her muscles start to give way; allowing me greater access into her depths with each push forward."
    "По мере того как она начинает расслабляться вокруг меня, я чувствую, как ее мышцы начинают поддаваться, позволяя мне с каждым толчком все больше проникать в ее глубины."

# game/script.rpy:617
translate rus cum_back_893fc7c5:

    # "Her moans turn from pained gasps to soft whimpers mixed with pleasure as our bodies find their rhythm together in this taboo dance."
    "Ее стоны превращаются из болезненных вздохов в тихое поскуливание, смешанный с удовольствием, когда наши тела находят свой ритм в этом запретном танце."

# game/script.rpy:618
translate rus cum_back_7d274a81:

    # pc "You're doing so well."
    pc "Ты умничка."

# game/script.rpy:619
translate rus cum_back_ad609ced:

    # "I whisper into her ear before nipping at her lobe playfully while increasing my tempo once again - making sure not to overwhelm her sensitive flesh with too much force or speed just yet."
    "Я шепчу ей на ухо, а затем игриво покусываю мочку, снова увеличивая темп - стараясь пока не перегружать ее чувствительную плоть слишком большой силой или скоростью."

# game/script.rpy:620
translate rus cum_back_7a751d65:

    # "I continue to thrust my road into Kristen's defenseless ass, feeling her body yield to my relentless advances."
    "Я продолжаю прокладывать себе дорогу в беззащитный анус Кристины, чувствуя, как ее тело поддается моим неумолимым натискам."

# game/script.rpy:621
translate rus cum_back_23c79475:

    # "Her moans grow louder and more passionate as she starts to enjoy the sensation of being filled in such a forbidden way."
    "Ее стоны становятся все громче и страстнее, поскольку она начинает наслаждаться ощущением, что ее заполняют таким запретным способом."

# game/script.rpy:622
translate rus cum_back_3baf5258:

    # "The tightness of her inexperienced sphincter grips me like a vice, sending waves of pleasure through my entire body with each powerful stroke."
    "Тугость ее неопытного сфинктера сжимает меня, как тиски, посылая волны удовольствия по всему телу с каждым мощным ударом."

# game/script.rpy:623
translate rus cum_back_96942895:

    # "As we move together in perfect harmony, our bodies slick with sweat and desire, I can feel myself getting closer to the edge."
    "Пока мы двигаемся вместе в идеальной гармонии, наши тела блестят от пота и желания, я чувствую, что приближаюсь к кульминации."

# game/script.rpy:624
translate rus cum_back_ce32af2b:

    # "Knowing that I want this experience to last, I slow down once again - drawing out each movement for maximum effect."
    "Я хочу, чтобы этот опыт длился долго и снова замедляюсь, вытягивая каждое движение для достижения максимального эффекта."

# game/script.rpy:625
translate rus cum_back_aa146084:

    # "My cock twitches inside her, aching for release but willing to wait until she finds her own climax first."
    "Мой член дергается внутри нее, жаждет разрядки, но готов подождать, пока она сама не достигнет оргазма."

# game/script.rpy:626
translate rus cum_back_e72aa7eb:

    # k "It… it feels so good."
    k "Это... почему это так приятно?"

# game/script.rpy:627
translate rus cum_back_260368a1:

    # "Kristen pants, her voice barely audible over the sound of our bodies slapping together."
    "Кристина задыхается, ее голос едва слышен за звуком наших тел, шлепающихся друг о друга."

# game/script.rpy:628
translate rus cum_back_6899dee1:

    # k "I never thought I'd enjoy this… but you make it feel incredible."
    k "Я никогда не думала, что мне это понравится... но с тобой это кажется невероятным."

# game/script.rpy:629
translate rus cum_back_8d9e104c:

    # pc "I'm glad you're enjoying it"
    pc "Рад услужить."

# game/script.rpy:630
translate rus cum_back_c8daaa7f:

    # "I say it with a wicked grin as our bodies continue their dance."
    "Я говорю это с лукавой ухмылкой, пока наши тела продолжают свой танец."

# game/script.rpy:631
translate rus cum_back_4c4e58a5:

    # "I lean in close to Kristen's ear, my breath hot against her skin as I whisper encouragingly:"
    "Я наклоняюсь к уху Кристины, мое дыхание обжигает ее кожу пока я шепчу ободряюще:"

# game/script.rpy:632
translate rus cum_back_f320585e:

    # pc "You're doing amazing. Just let go and enjoy the sensations."
    pc "Ты отлично справляешься. Просто отпусти себя и наслаждайся ощущениями."

# game/script.rpy:633
translate rus cum_back_fba2331b:

    # "As we continue our passionate embrace, I reach around her waist and guide her hips back towards me - angling her body for deeper penetration."
    "Пока мы продолжаем страстные объятия, я обхватываю ее за талию и направляю ее бедра назад к себе, наклоняя ее тело для более глубокого проникновения."

# game/script.rpy:634
translate rus cum_back_19891e8d:

    # "Our movements become more urgent now; driven by raw lust and desire that can no longer be contained or restrained."
    "Теперь наши движения становятся более настоятельными, ими движет необузданная похоть и желание, которые уже невозможно сдержать и обуздать."

# game/script.rpy:635
translate rus cum_back_3565e53b:

    # "My thrusts grow harder and faster, eliciting cries of pleasure from both of us as we hurtle towards climax together."
    "Мои толчки становятся все сильнее и быстрее, вызывая крики удовольствия у нас обоих, и мы вместе приближаемся к оргазму."

# game/script.rpy:636
translate rus cum_back_f2bb1bd4:

    # "The sound of our bodies slapping echoes through the room like a primal drumbeat - a testament to our unbridled passion."
    "Звук шлепков наших тел разносится по комнате, как первобытный барабанный бой - свидетельство нашей безудержной страсти."

# game/script.rpy:637
translate rus cum_back_3ebad97d:

    # "Kristen's eyes roll back in her head as she cries out:"
    "Глаза Кристины закатились от стона:"

# game/script.rpy:638
translate rus cum_back_dc25407b:

    # k "I'm going to cum!"
    k "Я кончааааю!"

# game/script.rpy:639
translate rus cum_back_e93c4da4:

    # "Her body tenses around me, squeezing my cock even tighter as her orgasm washes over her."
    "Ее тело напрягается вокруг меня, сжимая мой член еще сильнее, когда оргазм захлестывает ее."

# game/script.rpy:640
translate rus cum_back_e14d8f62:

    # "The sensation is too much for me to bear; I can feel myself reaching the edge as well."
    "Ощущения слишком сильны для меня, я чувствую, что тоже подхожу к краю."

# game/script.rpy:641
translate rus cum_back_2c837084:

    # "With a final powerful thrust, I bury myself deep inside her ass and release my load - filling her with warmth as we both shudder in ecstasy."
    "Последним мощным толчком я глубоко погружаюсь в ее попку и разряжаю свои яйца, наполняя ее теплом, пока мы оба содрогаемся в экстазе."

# game/script.rpy:642
translate rus cum_back_d7e1fb8f:

    # "Our bodies collapse together in a sweaty heap, spent but satisfied from our shared experience."
    "Наши тела слились в единый потный ком, измученные, но удовлетворенные совместным опытом."

# game/script.rpy:652
translate rus goodbye_kiss_657f65e2:

    # k "\"Was that good for you?\""
    k "Тебе было хорошо?"

# game/script.rpy:653
translate rus goodbye_kiss_54dea4ad:

    # "Her voice hoarse from our intense encounter. A small smile played on her lips as she waited for my answer."
    "Ее голос охрип от напряженния, но на губах играла улыбка. Она с надеждой ждет моего ответа."

# game/script.rpy:654
translate rus goodbye_kiss_1b4077ef:

    # "I nodded, still catching my breath."
    "Я кившул, всё ещё не в силах восстановить дыхание."

# game/script.rpy:655
translate rus goodbye_kiss_4475f470:

    # pc "\"Fuck yeah,\" I managed to get out between pants. \"You are amazing.\""
    pc "О, да! Ты потрясающая."

# game/script.rpy:656
translate rus goodbye_kiss_7c56a079:

    # "I pulled her into a tight embrace, nuzzling my face into her neck."
    "Я притянул ее к себе и крепко обнял, уткнувшись лицом в ее шею."

# game/script.rpy:657
translate rus goodbye_kiss_cba915c1:

    # pc "\"I gotta say,\" I said, running my fingers through her hair. \"That was one hell of a way to kick off my night.\""
    pc "Должен сказать, что это был чертовски хороший способ начать мою ночь."

# game/script.rpy:658
translate rus goodbye_kiss_f12b793c:

    # "I grabbed my clothes from where they had been discarded earlier and began dressing himself once again."
    "Я схватил свою одежду с того места, где она была выброшена ранее, и начал одеваться."

# game/script.rpy:659
translate rus goodbye_kiss_f55a4fee:

    # "The encounter with Kristen had been incredible, but there were more girls waiting for me at the bar - eager eyes looking for their own chance at an unforgettable experience like hers!"
    "Встреча с Кристиной была невероятной, но в баре меня ждали еще девушки - жаждущие получить свой собственный шанс на незабываемый опыт, как у нее!"

# game/script.rpy:660
translate rus goodbye_kiss_2a437dbd:

    # "I couldn't help but feel a twinge of excitement as I thought about it all."
    "При мысли обо всем этом меня охватило волнение."

# game/script.rpy:661
translate rus goodbye_kiss_951692a0:

    # pc "You take care now. And thanks for the night."
    pc "Спасибо за незабываемый вечер. Но мне уже надо идти."

# game/script.rpy:663
translate rus goodbye_kiss_0c052b1f:

    # "FIN"
    "FIN"

# game/script.rpy:669
translate rus rape_32f0424c:

    # "As my gaze locked onto Kristen, a sinister grin spread across my face."
    "Когда мой взгляд остановился на Кристен, по моему лицу расплылась зловещая ухмылка."

# game/script.rpy:670
translate rus rape_2c8aa451:

    # "With a swift motion, I pulled out a gun from my pocket and pointed it at her."
    "Быстрым движением я достал из кармана пистолет и направил его на нее."

# game/script.rpy:675
translate rus rape_9947c4c7:

    # pc "I'm going to rape you (growled menacingly)."
    pc "Я тебя изнасилую!"

# game/script.rpy:676
translate rus rape_a2d0f27e:

    # pc "Strip."
    pc "Раздевайся."

# game/script.rpy:677
translate rus rape_0bbc5ba3:

    # "The command was delivered with an air of authority that left no room for negotiation."
    "Приказ был произнесен властно, не оставляя места для переговоров."

# game/script.rpy:678
translate rus rape_a55cddb3:

    # "Kristen's eyes widened in terror."
    "Глаза Кристины расширились от ужаса."

# game/script.rpy:679
translate rus rape_c4d76bcd:

    # "Kristen's heart races as her mind tries to process the situation."
    "Сердце Кристен бешено колотится, пока она пытается осмыслить ситуацию."

# game/script.rpy:680
translate rus rape_6b0602f9:

    # "She swallows hard, eyes wide with fear, and pleads with me in a shaky voice:"
    "Она тяжело сглатывает, глаза расширены от страха, и умоляет меня дрожащим голосом:"

# game/script.rpy:681
translate rus rape_219a7085:

    # k "P-please… don't hurt me. I'll do whatever you want."
    k "П-пожалуйста... не делай мне больно. Я сделаю все, что ты захочешь."

# game/script.rpy:682
translate rus rape_4a295e5f:

    # "Tears well up in her eyes as she looks at the gun tremblingly, begging for mercy while trying not to make any sudden moves that might agitate me further."
    "Со слезами наворачивающимися на глаза, она в трепете смотрит на пистолет, умоляя о пощаде и стараясь не делать резких движений, которые могли бы меня спровоцировать."

# game/script.rpy:683
translate rus rape_5c11ac44:

    # "I chuckle darkly, a mix of amusement and arousal in my voice."
    "Я мрачно усмехаюсь, в моем голосе смешались веселье и возбуждение."

# game/script.rpy:684
translate rus rape_63a8f8ee:

    # pc "Good girl"
    pc "Хорошая девочка."

# game/script.rpy:685
translate rus rape_6e1b7326:

    # "I say it softly before sudden push on her towards the couch."
    "Я говорю это мягко, прежде чем внезапно подтолкнуть ее к дивану."

# game/script.rpy:686
translate rus rape_03753d78:

    # pc "Take off your clothes."
    pc "Снимай одежду."

# game/script.rpy:687
translate rus rape_c5a0304b:

    # "I commands again, this time with more force as I moves closer to her, the gun still trained on her trembling form."
    "Я снова командую, на этот раз с большей силой, приближаясь к ней, пистолет по-прежнему нацелен на ее дрожащую фигуру."

# game/script.rpy:688
translate rus rape_9f5d5db2:

    # "Kristen's hands shake as she slowly begins to unbutton her dress, tears streaming down her cheeks."
    "Руки Кристины дрожат, она медленно начинает расстегивать платье, по щекам текут слезы."

# game/script.rpy:689
translate rus rape_642c112d:

    # k "Please… just let me go…"
    k "Пожалуйста... просто отпусти меня..."

# game/script.rpy:690
translate rus rape_85767fb2:

    # "Kristen whispers between sobs, hoping that might make me reconsider my actions. But the look in my eyes tells her it won't be enough."
    "Кристен шепчет между всхлипами, надеясь, что это заставит меня пересмотреть свои действия. Но взгляд моих глаз говорит ей, что этого будет недостаточно."

# game/script.rpy:695
translate rus rape_a9fbb80d:

    # "The dress falls to the floor, revealing a black lace bra and matching panties that hug her curves tightly."
    "Платье ниспадает до пола, обнажая черный кружевной бюстгальтер и подходящие трусики, которые плотно облегают ее изгибы."

# game/script.rpy:696
translate rus rape_2992de0e:

    # "She takes a deep breath before starting to undo the clasp of her bra, her trembling fingers fumbling with the tiny hooks."
    "Она делает глубокий вдох и начинает расстегивать застежку бюстгальтера, дрожащими пальцами нащупывая крошечные крючки."

# game/script.rpy:697
translate rus rape_39720fd8:

    # "I'm scared, Anton!"
    "Ты меня пугаешь, Антон!"

# game/script.rpy:698
translate rus rape_3d6af53a:

    # "She whimpers, looking up at me beseechingly."
    "Она хнычет, умоляюще глядя на меня."

# game/script.rpy:699
translate rus rape_09fb1418:

    # "I watch her every move with a predatory gaze, my eyes glinting with lust and excitement."
    "Я слежу за каждым ее движением хищным взглядом и мои глаза блестят от вожделения и возбуждения."

# game/script.rpy:700
translate rus rape_b89e1a31:

    # pc "You're doing great."
    pc "Ты отлично справляешься. Раздевайся до конца."

# game/script.rpy:701
translate rus rape_24e22db6:

    # "I said it reassuringly, though there's an undercurrent of menace to my voice that sends shivers down Kristen's spine."
    "Я сказал это успокаивающе, хотя в моем голосе прозвучали нотки угрозы, от которых у Кристины по спине побежали мурашки."

# game/script.rpy:702
translate rus rape_dc261132:

    # pc "Now take of the rest, slut!"
    pc "Шевелись. Снимай всё, блядина!"

# game/script.rpy:703
translate rus rape_c33a3a64:

    # "Kristen's hands shake as she fumbles with the clasp of her bra, finally managing to undo it and letting it fall to the floor."
    "Руки Кристины дрожат, она возится с застежкой бюстгальтера, но в конце концов ей удается расстегнуть его и он падает на пол."

# game/script.rpy:704
translate rus rape_2d117e19:

    # "Her breasts are exposed now, heaving slightly in fear and anticipation."
    "Ее грудь обнажена и слегка вздымается от страха и предвкушения."

# game/script.rpy:705
translate rus rape_5ac0cf35:

    # "She looks up at I through tear-filled eyes, praying for mercy while also feeling a twisted thrill from this dangerous situation."
    "Она смотрит на меня полными слез глазами, моля о пощаде и одновременно испытывая извращенное возбуждение от этой опасной ситуации."

# game/script.rpy:706
translate rus rape_3a8ac811:

    # k "Please.. (she whispers softly) don't hurt me... much."
    k "Пожалуйста (слабым  шопотом) не делай мне больно..."

# game/script.rpy:707
translate rus rape_04d462aa:

    # "With shaking hands, Kristen reaches for the lace of her panties and slowly slides us down her legs."
    "Дрожащими руками Кристина дотягивается до кружева трусиков и медленно спускает их вниз по ногам."

# game/script.rpy:708
translate rus rape_0534fa2c:

    # "She feels exposed and vulnerable in front of I, but also aroused by the danger of it all."
    "Она чувствует себя незащищенной и уязвимой в присутствии меня, но в то же время ее возбуждает опасность всего этого."

# game/script.rpy:712
translate rus rape_cb62e402:

    # "Her heart races as she steps out of her underwear, standing before me completely naked."
    "Сердце бешено колотится, когда она вылезает из нижнего белья и предстает передо мной совершенно обнаженной."

# game/script.rpy:722
translate rus rape_9f401b50:

    # k "Please let me go... I begging you, Anton."
    k "Антон, я умоляю тебя... отпусти меня. Прошу тебя."

# game/script.rpy:723
translate rus rape_3fdc6f8b:

    # "She whispers again, looking up at me with pleading eyes."
    "Она снова шепчет, глядя на меня умоляющими глазами."

# game/script.rpy:724
translate rus rape_857ebd47:

    # k "Why are you doing this?"
    k "Зачем ты делаешь это со мной?"

# game/script.rpy:725
translate rus rape_de36654a:

    # "I smirk, taking a step closer to her."
    "Я ухмыляюсь, делая шаг ближе к ней."

# game/script.rpy:726
translate rus rape_f322a124:

    # pc "Because you're mine now,"
    pc "Потому что теперь, ты будешь принадлежать мне."

# game/script.rpy:727
translate rus rape_9816abf9:

    # "I growl, my voice low and rough."
    "Я рычу это низкии и грубыи голосом."

# game/script.rpy:728
translate rus rape_33d17c44:

    # "I grab her hair firmly, pulling her head back as I force my tongue onto her mouth in a deep, possessive kiss."
    "Я крепко хватаю ее за волосы, оттягивая ее голову назад, и проникаю языком в ее рот в глубоком, собственническом поцелуе."

# game/script.rpy:729
translate rus rape_222b01b5:

    # "My free hand roams over her body, tracing patterns on her skin before reaching between her legs."
    "Моя свободная рука блуждает по ее телу, вырисовывая узоры на ее коже, а затем тянется между ее ног."

# game/script.rpy:730
translate rus rape_c4e35532:

    # pc "You'll scream my name when I'm done with you!"
    pc "Когда я закончу с тобой, ты будешь кричать моё имя!"

# game/script.rpy:731
translate rus rape_3b072828:

    # "I whisper passionately against her lips, my fingers teasing her already wet pussy."
    "Я страстно шепчу ей в ухо, пока мои пальцы дразнят ее уже мокрую киску."

# game/script.rpy:732
translate rus rape_3802c6ed:

    # "My kiss is forceful and dominating, my tongue pushing past her lips and invading her mouth with a roughness that leaves no room for resistance."
    "Мой поцелуй - властный и доминирующий, мой язык проникает за ее губы и вторгается в ее рот с грубостью, не оставляющей места для сопротивления."

# game/script.rpy:733
translate rus rape_b1a7a23e:

    # "She can taste the alcohol on my breath mixed with my own musky scent as I holds her head still while my other hand roams lower, finding her wet folds once again."
    "Она чувствует запах алкоголя на моем дыхании, смешанный с моим собственным мускусным ароматом, пока я удерживаю ее голову в неподвижном состоянии, а моя вторая рука опускается ниже, снова нащупывая ее влажные складочки."

# game/script.rpy:734
translate rus rape_2097a07e:

    # "Without warning, I thrust two fingers deep into her core, causing Kristen to gasp."
    "Без предупреждения я ввел два пальца глубоко в ее лоно, заставив Кристен задыхаться."

# game/script.rpy:735
translate rus rape_f490ca6a:

    # "I chuckles darkly before pulling my fingers out, only to push them back in harder this time."
    "Я мрачно усмехнулся и вытащил пальцы, только для того, чтобы снова вставить их посильнее."

# game/script.rpy:736
translate rus rape_b1860e16:

    # pc "You like that, don't you?"
    pc "Тебе ведь это нравится, шлюха. Признавайся!"

# game/script.rpy:737
translate rus rape_6bca40e0:

    # "I growl, watching as she squirms under my touch."
    "Я рычу, наблюдая за тем, как она извивается под моими прикосновениями."

# game/script.rpy:738
translate rus rape_215abb1a:

    # "Kristen whimpers, trying to push away from me but unable to escape my grasp."
    "Кристен хнычет, пытаясь оттолкнуться от меня, но не в силах вырваться из моей хватки."

# game/script.rpy:739
translate rus rape_31f7aa9b:

    # "My fingers feel like they're on fire inside her, stretching and filling her in a way that makes her gasp for air."
    "Мои пальцы словно горят внутри нее, растягивая и наполняя ее так, что она задыхается."

# game/script.rpy:740
translate rus rape_52966eac:

    # "She nods weakly, tears streaming down her face as she tries not to choke on my tongue invading her mouth."
    "Она слабо кивает, слезы текут по ее лицу, когда она пытается не задохнуться от моего языка, проникающего в ее рот."

# game/script.rpy:741
translate rus rape_7537e589:

    # k "Yes. (she manages between moans) Please stop… please don't hurt me anymore!"
    k "Да. Только пожалуйста, остановись... пожалуйста, не делай мне больше больно!"

# game/script.rpy:742
translate rus rape_ab874195:

    # "I pull my fingers out of her, leaving her aching."
    "Я вытаскиваю из нее пальцы."

# game/script.rpy:771
translate rus throatfuck_5a64b90a:

    # "I push her down on the floor."
    "И прижимаю её к полу."

# game/script.rpy:772
translate rus throatfuck_26b0a594:

    # pc "Kneel, bitch! You'll beg for me."
    pc "На колени, сука! Умоляй чтобы я тебя трахнул!"

# game/script.rpy:773
translate rus throatfuck_ba835999:

    # "Kristen's heart races as she kneels before me, her eyes wide with fear and arousal."
    "Сердце Кристен бешено колотится, когда она опускается передо мной на колени, ее глаза расширены от страха и возбуждения."

# game/script.rpy:774
translate rus throatfuck_37f0cf4f:

    # pc "Now open your fucking moth wide!"
    pc "А теперь открой свою пасть и высунь язык!"

# game/script.rpy:775
translate rus throatfuck_6d1b634b:

    # "She opens her mouth obediently, feeling my hardness press against her lips."
    "Она послушно открывает рот, чувствуя, как головка моего члена, красная от напряжения, прижимается к ее губам."

# game/script.rpy:776
translate rus throatfuck_b3f7b1ae:

    # k "Please don't hurt me, Anton"
    k "Антон, только не делай мне больно, умоляю."

# game/script.rpy:777
translate rus throatfuck_e4b2ac22:

    # "She whispers again, her voice trembling."
    "Она снова шепчет, ее голос дрожит."

# game/script.rpy:778
translate rus throatfuck_13d36efb:

    # k "I'll do anything you want."
    k "Я сделаю как ты скажешь..."

# game/script.rpy:780
translate rus throatfuck_a1696a01:

    # "I smirk, pushing my cock into her mouth roughly, grabbing her hair and forcing my cock deeper down her throat."
    "Я ухмыляюсь, хватаю ее за волосы и проталкиваю свой член в ее рот."

# game/script.rpy:781
translate rus throatfuck_e7874c6c:

    # pc "That's right."
    pc "Вот так."

# game/script.rpy:782
translate rus throatfuck_77945665:

    # "Kristen gags on mmy rod, her eyes watering as she struggles to breathe around my thickness."
    "Кристен задыхается насаженная ртом на мой стержень, ее глаза слезятся, когда она пытается дышать."

# game/script.rpy:783
translate rus throatfuck_b81ab2e1:

    # pc "Take it all, whore!"
    pc "Целиком заглатывай, хуесоска ёбаная!"

# game/script.rpy:784
translate rus throatfuck_e194e2ac:

    # "I start to thrust in and out of her mouth with increasing speed."
    "Я начинаю входить и выходить из ее рта с нарастающей скоростью."

# game/script.rpy:785
translate rus throatfuck_174992c8:

    # "As my thrusts become more forceful, my hips slamming against her face."
    "Мои толчки становятся все более сильными, а мои бедра шлепают Кристину по лицу."

# game/script.rpy:786
translate rus throatfuck_1df98e83:

    # "Taking my full control, Kristen feels like she can't breathe around my cock filling every inch of her mouth and stretching it to its limit."
    "Подчинясь полностью моему контролю, Кристен чувствует, что не может дышать, когда мой член заполняет каждый сантиметр ее рта и растягивает его до предела."

# game/script.rpy:787
translate rus throatfuck_1354a0e3:

    # "She gags repeatedly on each thrust, tears streaming down her cheeks from the pain and humiliation."
    "При каждом толчке она раз за разом затыкает рот, слезы текут по ее щекам от боли и унижения."

# game/script.rpy:788
translate rus throatfuck_b2f7d90f:

    # "I growl like a wild animal, my hips moving faster as I pounds into her throat."
    "Я рычу как дикий зверь, мои бедра двигаются быстрее, когда я вдавливаю свой твёрдый хуй в ее напряженно пульсирующее горло."

# game/script.rpy:789
translate rus throatfuck_688c1f7c:

    # pc "That's it."
    pc "Да, вот так."

# game/script.rpy:790
translate rus throatfuck_6c987f57:

    # "I grabb her hair tighter to keep her in place. My other hand roams over her body, tracing patterns on her skin as I forcefully use her helolessly open throat."
    "Я крепко хватаю ее за волосы, чтобы удержать на месте. Другая рука блуждает по ее телу, вырисовывая узоры на коже, пока я с силой использую ее беспомощно открытое горло."

# game/script.rpy:791
translate rus throatfuck_2970ae02:

    # "Kristen whimpers with each thrust, feeling like she might choke on me at any moment."
    "Кристен хнычет с каждым толчком, чувствуя, что в любой момент может задохнуться."

# game/script.rpy:792
translate rus throatfuck_9a8eb79f:

    # "She tries to pull away but can't escape my grip or the intense pleasure that pulses through her body."
    "Она пытается вырваться, но не может освободиться ни от моей хватки, ни от сильного удовольствия, которое пульсирует в ее теле."

# game/script.rpy:793
translate rus throatfuck_a47c1d04:

    # "Kristen's eyes roll back in her head, a mix of pain and nausea coursing through her."
    "Кристен закатывает глаза, в них отражается боль и тошнота."

# game/script.rpy:794
translate rus throatfuck_e03d1d5c:

    # "She can feel my cock hitting the back of her throat with each thrust, forcing her to take me deeper than she ever thought possible."
    "Она чувствует, как мой член с каждым толчком ударяется о заднюю стенку ее глотки, заставляя ее принимать меня глубже, чем она когда-либо думала, что это возможно."

# game/script.rpy:795
translate rus throatfuck_7fc06c38:

    # "Her body shudders as she comes undone around me, moaning loudly despite the cock in her mouth."
    "Ее тело содрогается, когда она расстягивается вокруг меня, и она громко стонет, несмотря на полный рот."

# game/script.rpy:796
translate rus throatfuck_12d0bda2:

    # k "Please (she manages between gasps for air) stop… I can't take it anymore!"
    k "Пожалуйста, остановись... (удаётся ей вымолвить, пока она пытается вдохнуть) Я больше не могу!"

# game/script.rpy:797
translate rus throatfuck_c47611b5:

    # "I chuckle darkly, pulling my dick out of her mouth with a pop."
    "Я мрачно усмехаюсь, с шумом вытаскивая член из ее рта."

# game/script.rpy:798
translate rus throatfuck_2e343fc5:

    # "I smirk down at her, my eyes glinting with satisfaction as I watches her gag and cough."
    "Я ухмыляюсь, глядя на нее, мои глаза блестят от удовлетворения, когда я наблюдаю за ее рвотными позывами и кашлем."

# game/script.rpy:806
translate rus forced_sex_fe6e5ebe:

    # pc "On your back, whore!"
    pc "На спину, шлюха!"

# game/script.rpy:807
translate rus forced_sex_61775f54:

    # "I'm taking charge, shoving her down onto the couch."
    "Я беру на себя ответственность, пихаю ее на диван."

# game/script.rpy:808
translate rus forced_sex_2ebb9376:

    # "Girl complied, her body trembling with fear as I towered over her."
    "Девушка подчинилась, ее тело дрожало от страха, когда я возвышался над ней."

# game/script.rpy:809
translate rus forced_sex_88fd6d75:

    # "I positioned myself between her legs, admiring the sight of her spread out before me - vulnerable and exposed."
    "Я расположился между ее ног, любуясь тем, как она раскинулась передо мной - уязвимая и обнаженная."

# game/script.rpy:810
translate rus forced_sex_d3e6a39e:

    # "Despite her fear and humiliation, her soft pink labia glisten with traitorous moisture."
    "Несмотря на страх и унижение, ее нежно-розовые половые губы предательски блестят."

# game/script.rpy:811
translate rus forced_sex_78e2f8d9:

    # pc "You're so fucking sexy when you're scared."
    pc "Ты чертовски сексуальна, когда напугана."

# game/script.rpy:812
translate rus forced_sex_192ada7f:

    # "I smiled, running a finger along her inner thigh. Kristen flinched at my touch but didn't dare to resist."
    "Я улыбнулся, проведя пальцем по внутренней стороне ее бедра. Кристен вздрогнула от моего прикосновения, но не осмелилась сопротивляться."

# game/script.rpy:813
translate rus forced_sex_38fd1460:

    # "Kristen's heart pounded in her chest as she lay there, helpless and exposed."
    "Сердце Кристины колотилось в груди, пока она лежала передо мной, беспомощная и обнаженная."

# game/script.rpy:814
translate rus forced_sex_a7aa66e9:

    # "She could feel the heat of my body above her, my breath hot on her neck as I traced a finger along her inner thigh."
    "Она чувствовала жар моего тела над собой, мое дыхание обжигало ее шею, когда я проводил пальцем по внутренней стороне ее бедра."

# game/script.rpy:815
translate rus forced_sex_e1b4be63:

    # "Despite her fear, she couldn't help but feel a twinge of arousal at being so completely under my control."
    "Несмотря на свой страх, она не могла не почувствовать прилив возбуждения от того, что находится под моим полным контролем."

# game/script.rpy:816
translate rus forced_sex_b485e6b4:

    # "Please… (she whispered, her voice barely audible) Don't do this to me."
    "Пожалуйста... (она прошептала, едва слышно) Не делай этого со мной."

# game/script.rpy:820
translate rus forced_sex_096f0029:

    # "Ignoring her plea, I rubbed my cock against her entrance, teasing her with the tip before thrusting inside without warning."
    "Не обращая внимания на ее мольбы, я потерся членом о ее вход, дразня ее кончиком, прежде чем без предупреждения ворваться внутрь."

# game/script.rpy:821
translate rus forced_sex_6c6ab909:

    # "Kristen cried out in pain as I filled her completely, her body unprepared for my size."
    "Кристен вскрикнула от боли, когда я полностью заполнил ее, ее тело не было готово к моим размерам."

# game/script.rpy:822
translate rus forced_sex_64e28220:

    # "I began to pound into her relentlessly, each stroke driving deeper and harder than the last."
    "Я начал неустанно вбиваться в нее, каждый удар был глубже и сильнее предыдущего."

# game/script.rpy:823
translate rus forced_sex_c2a4cc66:

    # "Her cries turned to moans as her body betrayed her, responding to the rough treatment despite her terror."
    "Ее крики перешли в стоны, когда тело предало ее, отвечая возбуждением на грубое обращение, невзирая на искренний ужас."

# game/script.rpy:824
translate rus forced_sex_b03e1faf:

    # pc "You like it, don't you?"
    pc "Что, нравится?"

# game/script.rpy:825
translate rus forced_sex_ac83b506:

    # "Kristen's body arched off the bed as I pounded into her, my words sending a shiver down her spine."
    "Тело Кристины выгнулось дугой, когда я вошел в нее, а мои слова вызвали дрожь по ее позвоночнику."

# game/script.rpy:826
translate rus forced_sex_66363d2b:

    # "She couldn't deny the pleasure that was building within her, but she refused to give me the satisfaction of hearing her admit it."
    "Она не могла отрицать удовольствие, которое росло в ней, но отказывалась доставить мне удовольствие услышать ее признание."

# game/script.rpy:827
translate rus forced_sex_fc0b3a91:

    # k "No… stop!"
    k "Нет... прекрати!"

# game/script.rpy:828
translate rus forced_sex_7c5f900f:

    # "She gasped between moans, trying desperately to maintain some semblance of control. But with each thrust, her resolve weakened further."
    "Она задыхалась между стонами, отчаянно пытаясь сохранить хоть какое-то подобие контроля. Но с каждым толчком ее решимость все больше ослабевала."

# game/script.rpy:829
translate rus forced_sex_c4451760:

    # "I laughed, enjoying her struggle against the pleasure I was giving her."
    "Я рассмеялся, наслаждаясь ее борьбой с удовольствием, которое я ей доставлял."

# game/script.rpy:830
translate rus forced_sex_b5fcc37e:

    # "I grabbed her hips, holding her in place as I continued to drive into her with brutal force."
    "Я схватил ее за бедра, удерживая на месте, пока продолжал входить в нее с неистовой силой."

# game/script.rpy:831
translate rus forced_sex_167a28db:

    # pc "You're so tight! I bet you've never been fucked like this before."
    pc "Ты такая тугая! Держу пари, тебя еще никогда так не ебали."

# game/script.rpy:832
translate rus forced_sex_382e55db:

    # "Kristen's eyes widened as she felt my grip on her hips, my words sending a jolt of humiliation through her."
    "Глаза Кристен расширились, когда она почувствовала мою хватку на своих бедрах, а мои слова вызвали у нее приступ унижения."

# game/script.rpy:833
translate rus forced_sex_04333576:

    # "She knew I was right - she had never experienced anything like this before, and the realization only heightened her humiliating arousal."
    "Она знала, что я прав - она никогда не испытывала ничего подобного, и осознание этого только усиливало ее унизительное возбуждение."

# game/script.rpy:834
translate rus forced_sex_bf1c738b:

    # k "N-no!"
    k "Н-нет!"

# game/script.rpy:835
translate rus forced_sex_2aefb63b:

    # "Kristen stammered, her voice barely audible over the sound of our bodies slapping together."
    "Кристен заикалась, ее голос был едва слышен из-за звука наших тел, шлепающихся друг о друга."

# game/script.rpy:836
translate rus forced_sex_cc7388a1:

    # k "Please… Anton! Let me go… ah..."
    k "Пожалуйста, Антон! Отпусти меня... ай!"

# game/script.rpy:837
translate rus forced_sex_e6f7016f:

    # "My eyes locked on Kristen's face as I continued to pound into her. I could see the conflict within her - the desire to resist me warring with the pleasure I was giving her."
    "Мои глаза были устремлены на лицо Кристен, пока я продолжал входить в нее. Я видел, как внутри нее разгорается конфликт: желание сопротивляться мне боролось с удовольствием, которое я ей доставлял."

# game/script.rpy:838
translate rus forced_sex_515ecc00:

    # pc "You enjoying to be forced like this, admit it."
    pc "Вам шлюхам всегда нравится, когда вас заставляют, признай это."

# game/script.rpy:839
translate rus forced_sex_cd385997:

    # "Kristen's breath came in ragged gasps as she felt herself teetering on the edge of orgasm."
    "Кристен задыхалась, чувствуя, что находится на грани оргазма."

# game/script.rpy:840
translate rus forced_sex_b8cc5709:

    # k "I dont!"
    k "Иди к черту!"

# game/script.rpy:841
translate rus forced_sex_3cfdcd47:

    # "Poor girl gasped, her voice barely audible over the sound of our bodies slapping together."
    "Бедная девочка задыхалась, ее голос был едва слышен за звуком наших тел, шлепающихся друг о друга."

# game/script.rpy:842
translate rus forced_sex_fa3cdf35:

    # k "I don't want this… ah..."
    k "Мне это отвра... ай! ...отвратительно!"

# game/script.rpy:843
translate rus forced_sex_9b5f1b72:

    # "I laughed, my grip on her hips tightening as I continued to fuck her relentlessly."
    "Я рассмеялся, крепче сжимая ее бедра, продолжая неустанно трахать ее."

# game/script.rpy:844
translate rus forced_sex_af9fbb1b:

    # "I could feel her body responding to me despite her protests, and it only fueled my desire further."
    "Я чувствовал, как ее тело отвечает мне, несмотря на ее протесты, и это только разжигало мое желание еще больше."

# game/script.rpy:845
translate rus forced_sex_6e527a08:

    # pc "You're a liar! But your body tells the truth."
    pc "Мелкая, лживая блядь! Хотя бы твоё тело говорит правду."

# game/script.rpy:846
translate rus forced_sex_2916467b:

    # pc "And the truth is, you're a filthy, humiliation-enjoying slut!"
    pc "А правда в том, что ты грязная, наслаждающаяся унижениями мокрощель!"

# game/script.rpy:847
translate rus forced_sex_96b18f4c:

    # "Kristen's eyes squeezed shut as she felt the first waves of pleasure wash over her following my dirty words."
    "Кристен зажмурила глаза, почувствовав первые волны удовольствия, нахлынувшие на нее после моих грязных слов."

# game/script.rpy:848
translate rus forced_sex_82b2a360:

    # "She couldn't deny it any longer - her body was responding to my brutal thrusts, betraying her every attempt to resist me."
    "Она не могла больше отрицать этого - ее тело отзывалось на мои жестокие толчки, предавая все ее попытки сопротивляться мне."

# game/script.rpy:849
translate rus forced_sex_ecc7e8f5:

    # k "No… please… I can't… I can't stop it…"
    k "Нет... пожалуйста... я не могу... я не выдержу..."

# game/script.rpy:850
translate rus forced_sex_76da6ce6:

    # "My grin widened as I felt Kristen's body surrender to the pleasure, her words music to my ears. I pounded into her even harder, driving her closer and closer to the edge."
    "Моя ухмылка расширилась, когда я почувствовал, как тело Кристен отдается наслаждению, а ее слова стали музыкой для моих ушей. Я вбивался в нее еще сильнее, подталкивая ее все ближе и ближе к краю."

# game/script.rpy:851
translate rus forced_sex_d29ccdd8:

    # pc "That's it! Give in to it, whore!"
    pc "Вот так! Покажи мне свою блядскую сущность, пизда!"

# game/script.rpy:852
translate rus forced_sex_c1e7c1c2:

    # "Kristen's body arched off the bed as she felt the first waves of orgasm crash over her."
    "Тело Кристен выгнулось дугой, когда она почувствовала, как на нее накатывают первые волны оргазма."

# game/script.rpy:853
translate rus forced_sex_73025e84:

    # "She cried out, her voice mingling with my grunts as I continued to drive into her relentlessly."
    "Она вскрикнула, ее голос смешался с моим утробным рычанием, когда я продолжал неустанно входить в нее."

# game/script.rpy:854
translate rus forced_sex_d13839ad:

    # k "Aaaahhh... fuuuuck!"
    k "Аааааххх... бляяяяядь!"

# game/script.rpy:855
translate rus forced_sex_34ef59ef:

    # "I laughed, enjoying her helplessness as I continued to fuck her through her orgasm. I could feel my own release building, but I knew I could last much longer in needed."
    "Я хохотал, наслаждаясь ее беспомощностью, пока продолжал трахать ее оргазмирующую киску. Я чувствовал, как нарастает моя собственная разрядка, но знал, что смогу продержаться гораздо дольше."

# game/script.rpy:856
translate rus forced_sex_3bec0601:

    # pc "You're mine, say it."
    pc "Ты принадлежишь мне. Скажи это!"

# game/script.rpy:857
translate rus forced_sex_0a56aac2:

    # "Kristen's eyes squeezed shut as she felt another wave of pleasure wash over her."
    "Кристина зажмурила глаза, чувствуя, как ее захлестывает новая волна наслаждения."

# game/script.rpy:858
translate rus forced_sex_a9a95548:

    # "She couldn't deny it any longer - I had complete control over her body, and there was nothing she could do to stop me."
    "Она не могла больше отрицать этого - я полностью контролировал ее тело, и она ничего не могла сделать, чтобы остановить меня."

# game/script.rpy:859
translate rus forced_sex_830aa00d:

    # "I-I'm yours..."
    "Я... я твоя..."

# game/script.rpy:860
translate rus forced_sex_19855a64:

    # "She gasped, the admission ripped from her throat as her orgasm subsided."
    "Она задыхалась, признание вырвалось из ее горла, когда оргазм утих."

# game/script.rpy:861
translate rus forced_sex_195815f8:

    # k "Enough… spare me... please…"
    k "Хватит... пощади меня... пожалуйста..."

# game/script.rpy:868
translate rus anal_rape_bdfc9382:

    # pc "Now get on all fours and spread your ass for me, bitch!"
    pc "А теперь встань на четвереньки и раздвинь мне задницу, сучка!"

# game/script.rpy:869
translate rus anal_rape_0ca7495b:

    # "Kristen's heart races as she complies, her body shaking with fear and arousal. She gets on all fours, spreading her legs wide to expose herself fully to me."
    "Сердце Кристен бешено колотится, когда она подчиняется, ее тело дрожит от страха и возбуждения. Она встает на четвереньки, широко раздвигая ноги, чтобы полностью обнажить себя передо мной."

# game/script.rpy:870
translate rus anal_rape_0aeabd8b:

    # k "Please, not in the ass."
    k "Пожалуйста, только не попку."

# game/script.rpy:871
translate rus anal_rape_8032643b:

    # "She whimpers again, looking back at me through tear-filled eyes."
    "Она снова хнычет, глядя на меня глазами, полными слез."

# game/script.rpy:872
translate rus anal_rape_40532cd5:

    # k "I've never done that. I'm afraid… I'll do anything you want, just don't touch my tushy."
    k "Я никогда этого не делала. Я боюсь... Я сделаю все, что ты захочешь, только в попу не еби."

# game/script.rpy:873
translate rus anal_rape_b357000b:

    # "I chuckle darkly, my eyes glinting with lust and amusement."
    "Я мрачно усмехаюсь, в моих глазах сверкают похоть и веселье."

# game/script.rpy:874
translate rus anal_rape_70bd18e5:

    # pc "You'll do anything I want? Even if it means taking my cock in your ass?"
    pc "Ты сделаешь все, что я захочу? Даже если это означает принять мой член в свою задницу?"

# game/script.rpy:875
translate rus anal_rape_bc50e41a:

    # "I step closer to her, running a finger along the crack of her ass before pressing against her entrance."
    "Я подхожу к ней ближе, провожу пальцем по трещине ее попки, а затем прижимаюсь к ее входу."

# game/script.rpy:876
translate rus anal_rape_286329b5:

    # "Kristen flinches but doesn't move away, too scared to resist any further."
    "Кристен вздрагивает, но не отстраняется, слишком напуганная, чтобы сопротивляться дальше."

# game/script.rpy:877
translate rus anal_rape_8fdb7df9:

    # pc "Beg for it!"
    pc "Умоляй об этом, тварь!"

# game/script.rpy:878
translate rus anal_rape_705287ed:

    # "I slapped her ass hard, to reinforce my command."
    "Я сильно шлепнул ее по заднице, чтобы подкрепить свой приказ."

# game/script.rpy:879
translate rus anal_rape_86ce0500:

    # "Kristen whimpers, feeling my dickhead against her tight hole."
    "Кристина хнычет, чувствуя, как мой член упирается в ее тугую дырочку."

# game/script.rpy:880
translate rus anal_rape_e0536800:

    # k "Please… don't do this to me..."
    k "Пожалуйста, Антон... не поступай со мной так!"

# game/script.rpy:881
translate rus anal_rape_42f94103:

    # "She begs, her voice shaking with fear and humiliation."
    "Она умоляет, ее голос дрожит от страха и унижения."

# game/script.rpy:882
translate rus anal_rape_74abe449:

    # k "I'll do anything else… just not that."
    k "Я сделаю все, что угодно... только не это."

# game/script.rpy:883
translate rus anal_rape_f6e7a2b7:

    # "I laugh, a cruel sound that sends shivers down Kristen's spine."
    "Я смеюсь - жестокий звук, от которого по позвоночнику Кристины пробегают мурашки."

# game/script.rpy:884
translate rus anal_rape_c75261fc:

    # pc "You think you have a choice?"
    pc "Думаешь у тебя есть право кочевряжиться?"

# game/script.rpy:888
translate rus anal_rape_5b7aa832:

    # "I positions myself behind her, lining up my cock with her ass. With one swift motion, I thrust forward, burying it deep inside her unprepared hole."
    "Я встал позади нее, прижав свой член к ее анусу. Одним быстрым движением я подался вперед, глубоко погружая его в сжавшуюся от страха, девственную дырочку."

# game/script.rpy:889
translate rus anal_rape_61f20ebe:

    # "Kristen screams in pain as my hard rod rams in her butthole, stretching it beyond anything she's ever experienced before."
    "Кристина кричит от боли, когда мой твердый стержень впивается в ее попку, растягивая ее так, как она еще никогда не испытывала."

# game/script.rpy:890
translate rus anal_rape_0ca08e93:

    # pc "Now you're mine!"
    pc "Теперь твоя жопа принадлежит мне!"

# game/script.rpy:891
translate rus anal_rape_3ecab73c:

    # "I'm pulling out slowly before slamming back in again."
    "Я медленно вытаскиваю свой хер, а затем снова двигяю его вперёд."

# game/script.rpy:892
translate rus anal_rape_3902ca6a:

    # "Kristen screams in pain as I sodomize her roughtly, feeling like she's being torn apart."
    "Кристен кричит от боли, когда я грубо содомирую ее, чувствуя, будто её разрывают на части."

# game/script.rpy:893
translate rus anal_rape_40219a2f:

    # "Tears stream down her face as I continue to pound into her ass relentlessly, each stroke sending shockwaves of agony through her body."
    "Слезы текут по ее лицу, а я продолжаю без устали долбить ее попку, каждый удар посылает по ее телу волны агонии."

# game/script.rpy:894
translate rus anal_rape_7d3f46a9:

    # k "Aaaaa! Please… stop… It hurts so much…"
    k "Ааааа! Пожалуйста... остановись... Это так больно..."

# game/script.rpy:895
translate rus anal_rape_7994323a:

    # "I grunt, my breath coming in ragged gasps as I continues to pound into her."
    "Я хриплю, мое дыхание сбивается на рваные вдохи, когда я продолжаю в нее входить."

# game/script.rpy:896
translate rus anal_rape_e2f7202b:

    # pc "You're so fucking tight... The fist time is allways the best!"
    pc "Ты такая чертовски тугая. Первый раз всегда самый вкусный... для меня!"

# game/script.rpy:897
translate rus anal_rape_1a5cf2a1:

    # "My fingers digging into her hips as I forcefully impale her on my meaty stake."
    "Мои пальцы впиваются в ее бедра, когда я с силой насаживаю ее на свой мясистый кол."

# game/script.rpy:898
translate rus anal_rape_cd141f12:

    # pc "You're gonna take all form me, bitch!"
    pc "Ты получишь у меня, сучка!"

# game/script.rpy:899
translate rus anal_rape_3a016dd9:

    # "Kristen's eyes roll back in her head as I continues to thrust into her, the pain becoming too much to bear."
    "Кристина закатывает глаза, а я продолжаю входить в нее, делая боль в её осквернённой заднице невыносимой."

# game/script.rpy:900
translate rus anal_rape_6b46fb1f:

    # "She feels like she's going to pass out from the intensity of it all, but I show no signs of stopping."
    "Ей кажется, что она сейчас потеряет сознание от интенсивности происходящего, но я не подаю признаков остановки."

# game/script.rpy:901
translate rus anal_rape_ef3e08ef:

    # k "Please… I can't…"
    k "Пожалуйста... Я не могу..."

# game/script.rpy:902
translate rus anal_rape_ed44f685:

    # "She whimpers weakly, her body shaking with each brutal stroke."
    "Она слабо хнычет, ее тело содрогается от каждого жестокого удара."

# game/script.rpy:903
translate rus anal_rape_cd041cf0:

    # "My pace quickens, my breath coming in ragged gasps as I feels himself getting closer to the edge."
    "Мой темп ускоряется, дыхание становится рваным, когда я чувствую, что приближюсь к оргазму."

# game/script.rpy:911
translate rus cum_throat_49cc6e96:

    # pc "You'll take everything I give you!"
    pc "Ты примешь всё что я тебе дам, шлюха!"

# game/script.rpy:912
translate rus cum_throat_c08e4e4c:

    # pc "I'm going to fuck your little slutty mouth as hard as I please."
    pc "Я выебу твой мекий блядский рот так сильно, как поделаю."

# game/script.rpy:913
translate rus cum_throat_30edbfb4:

    # pc "I make you swallow my load and thank me for it, you bitch!"
    pc "Я заставлю тебя проглотить всё до капли и поблагодарить меня за это, сучка!"

# game/script.rpy:914
translate rus cum_throat_06a04366:

    # "My cock is hard and throbbing, ready to be pushed inside her mouth again."
    "Мой хер твердый и пульсирующий, полностю готов к тому, чтобы снова оказаться у нее во рту."

# game/script.rpy:915
translate rus cum_throat_1c84ec92:

    # "I grabs her hair roughly, pulling her head back before pushing my thick shaft between her lips once more."
    "Я грубо схватил ее за волосы и откинул голову назад, а затем снова ввел свой толстый вал между ее дрожащими губами."

# game/script.rpy:916
translate rus cum_throat_80fa02c9:

    # "She gags on me immediately as I begins to thrust in and out of her throat with long strokes that make her eyes water."
    "Она сразу же задыхается, когда я начинаю входить и выходить из ее горла длинными толчками, от которых у нее на глаза наворачиваются слёзы."

# game/script.rpy:917
translate rus cum_throat_e24c9830:

    # "Kristen whimpers, feeling like she might vomit as I takes her again. She tries to pull away but my grip is too strong."
    "Кристина хнычет, чувствуя, что ее может вырвать, когда я снова беру ее. Она пытается вырваться, но моя хватка слишком сильна."

# game/script.rpy:918
translate rus cum_throat_aad5066c:

    # "Her eyes water and her throat burns with each thrust of my cock, making it difficult for her to breathe or swallow properly."
    "Ее глаза слезятся, а горло горит от каждого толчка моего члена, из-за чего ей трудно дышать и глотать."

# game/script.rpy:919
translate rus cum_throat_30569f7d:

    # k "Please (she begs desperately) I can't take this…"
    k "Пожалуйста (отчаянно умоляет) я не могу этого вынести..."

# game/script.rpy:920
translate rus cum_throat_1ff433a4:

    # "My hips moving faster as I force my cock deeper into her throat."
    "Мои бедра двигаются быстрее, когда я проталкиваю свой член все глубже в ее горло."

# game/script.rpy:921
translate rus cum_throat_0bb47415:

    # pc "You'll take it all."
    pc "Ты примешь его весь."

# game/script.rpy:922
translate rus cum_throat_1ccdbd07:

    # "I grabs her hair tighter, enjoying the way she squirms under my touch, pulling her head back even further until she can feel me hitting the back of her throat with each thrust."
    "Я крепко хватаю ее за волосы, наслаждаясь тем, как она извивается под моими прикосновениями, и оттягиваю ее голову назад еще больше, пока она не чувствует, как я всаживаюсь в ее горло с каждым толчком."

# game/script.rpy:923
translate rus cum_throat_fdd68a31:

    # pc "Take it like a good little slut!"
    pc "Покажи мне какая ты послушная маленькая хуесоска!"

# game/script.rpy:924
translate rus cum_throat_2716e648:

    # "Kristen gags and chokes on me, her body shaking with each thrust. She feels like she's going to pass out from lack of oxygen but can't escape my tight grip."
    "Кристен задыхается и захлебывается, ее тело сотрясается от каждого толчка. Она чувствует, что сейчас потеряет сознание от недостатка кислорода, но не может вырваться из моей крепкой хватки."

# game/script.rpy:925
translate rus cum_throat_30f6bfb8:

    # "Tears stream down her face as she tries to beg for mercy once more:"
    "Слезы текут по ее лицу, когда она снова пытается умолять о пощаде:"

# game/script.rpy:926
translate rus cum_throat_84af7111:

    # k "Please… I can't breathe…"
    k "Пожалуйста... Я задыхаюсь..."

# game/script.rpy:927
translate rus cum_throat_13d2716b:

    # pc "Then stop whining and make me come."
    pc "Тогда перестань ныть и заставь меня кончить."

# game/script.rpy:928
translate rus cum_throat_c7124055:

    # "I roared my words mercilessly as Kristen hastily tried to inhale."
    "Я безжалостно прорычал свои слова, пока Кристен поспешно пыталась вдохнуть."

# game/script.rpy:929
translate rus cum_throat_5d9558f4:

    # pc "It'll be over as soon as you swallow every last drop of what I've prepared for you."
    pc "Все закончится, как только ты проглотишь все до последней капли, что я для тебя приготовил."

# game/script.rpy:930
translate rus cum_throat_2c2f2666:

    # "Kristen whimpers, feeling like she's drowning on my cock. She tries to focus on making me come, bobbing her head faster and sucking harder in an attempt to end this nightmare."
    "Кристен хнычет, чувствуя, что мир плывёт перед её глазами. Она пытается сосредоточиться на том, чтобы заставить меня кончить, быстрее мотает головой и сильнее сосет, пытаясь покончить с этим кошмаром."

# game/script.rpy:931
translate rus cum_throat_a7602749:

    # "Her eyes roll back in her head as I continues to thrust into her throat relentlessly."
    "Ее глаза закатились назад, пока я продолжал безжалостно долбить ее в глотку."

# game/script.rpy:932
translate rus cum_throat_07cfc0c7:

    # k "Khah… I need air…"
    k "Кхаа... Нужен... Воздух..."

# game/script.rpy:933
translate rus cum_throat_9a1d352d:

    # "I grunt, feeling myself getting closer to the edge. I thrusts even harder into her throat, enjoying the way she gags and chokes on my meaty dick."
    "Я практически хрюкаю, чувствуя, что приближаюсь к краю. Я еще сильнее вонзаюсь в ее горло, наслаждаясь тем, как она задыхается и захлебывается моим мясистым хреном."

# game/script.rpy:934
translate rus cum_throat_d348df53:

    # pc "Almost there!"
    pc "Уже почти там, сука!"

# game/script.rpy:935
translate rus cum_throat_a17e7e83:

    # "My hips moving even faster as I nearing my release."
    "Мои бедра задвигались еще быстрее, приближая меня к пиковому наслаждению."

# game/script.rpy:936
translate rus cum_throat_0a67ff2e:

    # pc "You're going to swallow it all, dirty coocksuking slut!"
    pc "Ты проглотишь всё до капли, грязная шлюха!"

# game/script.rpy:937
translate rus cum_throat_4996945d:

    # "My cock throbs in a girls mouth, pulsing with each beat of my heart."
    "Мой член раздувается во рту девушки, пульсируя с каждым ударом моего сердца."

# game/script.rpy:938
translate rus cum_throat_4a736cae:

    # "Kristen can feel me getting closer to the edge as I thrust harder into her throat. She tries to keep up, desperate for it to be over so she can breathe again."
    "Кристен чувствует, как я приближаюсь к завершающему аккорду и все сильнее вбиваю хер в ее горло. Она старается не отставать, отчаянно желая, чтобы все закончилось и она снова могла дышать."

# game/script.rpy:939
translate rus cum_throat_737e32bc:

    # "I groan, my body tensing as I am about to come."
    "Я стону, мое тело напрягается, как будто я вот-вот кончу."

# game/script.rpy:940
translate rus cum_throat_130e14a5:

    # "I grabs Kristen's hair tightly, holding her in place while I give her one last full depth thrust, releasing my load into her mouth."
    "Я крепко хватаю Кристен за волосы, удерживая ее на месте, пока делаю последний толчок на всю глубину, выпуская свою кончу в ее рот."

# game/script.rpy:941
translate rus cum_throat_011b95f3:

    # pc "Swallow it, fucking bitch!"
    pc "А теперь глотай, сука ёбаня!"

# game/script.rpy:942
translate rus cum_throat_dcdbe65c:

    # "I hold her head impaled on my cock, watching as she gags on my hot, sticky cum."
    "Я держу ее голову на своем члене, наблюдая, как она судорпожно глотает мою горячую, липкую сперму."

# game/script.rpy:943
translate rus cum_throat_69fcd2e1:

    # "Kristen gags on my cum, feeling it fill her mouth and spill down her chin."
    "Кристина глотает мою сперму, чувствуя, как она заполняет ее рот и растекается по подбородку."

# game/script.rpy:944
translate rus cum_throat_eb09eff7:

    # "She tries to swallow as much as she can but some of it drips out, running down her neck and onto her breasts."
    "Она пытается проглотить как можно больше, но часть вытекает, стекая по шее и попадая на грудь."

# game/script.rpy:945
translate rus cum_throat_b7ef5024:

    # "Her eyes water from the pain and humiliation as she struggles to breathe around my dick still lodged firmly in her throat."
    "Ее глаза слезятся от боли и унижения, она пытается дышать, когда мой член заканчивает разряжаться глубоко в ее горло."

# game/script.rpy:946
translate rus cum_throat_96dc08ee:

    # "I pull it out of her mouth, smirking as I watches her gasp for air."
    "Я наконец освобожюаю её рот и ухмыляюсь, глядя, как она с хлюпаньем глотает воздух откашливаясь слюной и спермой."

# game/script.rpy:947
translate rus cum_throat_617640cc:

    # "Wiping my cum from her chin with my thumb, I bringing it to her lips and forcing her to taste it."
    "Вытерев большим пальцем свою сперму с ее подбородка, я поднес пальцы к ее губам и заставил попробовать на вкус."

# game/script.rpy:948
translate rus cum_throat_449f43ab:

    # pc "You're mine now."
    pc "Ты моя хуесоска."

# game/script.rpy:949
translate rus cum_throat_7a4dc4db:

    # "I'm looking down on her, enjoying the look of defeat in her wet grey eyes."
    "Я смотрю на нее сверху вниз, наслаждаясь выражением поражения в ее мокрых от слёз серых глазах."

# game/script.rpy:950
translate rus cum_throat_f241b2ba:

    # pc "And you'll never forget it."
    pc "И ты этогот никогда не забудешь."

# game/script.rpy:951
translate rus cum_throat_7c91f200:

    # "Kristen whimpers, tasting her own humiliation as I forces her to lick my cum from my thumb."
    "Кристен рыдает, ощущая вкус собственного унижения, когда я заставляю ее слизывать мою сперму с пальцев."

# game/script.rpy:952
translate rus cum_throat_8018db96:

    # "She feels broken and used, her body shaking with fear and arousal as she looks up at me through tear-filled eyes."
    "Она чувствует себя сломленной и использованной, ее тело дрожит от страха и возбуждения, когда она смотрит на меня глазами, полными слез."

# game/script.rpy:953
translate rus cum_throat_70449a59:

    # "Please… let me go!"
    "Пожалуйста... отпусти меня!"

# game/script.rpy:954
translate rus cum_throat_e08b8d20:

    # "She begs once more, hoping that this nightmare will finally end."
    "Она снова умоляет, надеясь, что этот кошмар наконец закончится."

# game/script.rpy:955
translate rus cum_throat_700db80d:

    # "I smirks, enjoying the look of defeat on her face."
    "Я ухмыляюсь, наслаждаясь выражением беспомощности на ее красивом лице."

# game/script.rpy:956
translate rus cum_throat_f6ccb289:

    # pc "I'll let you go, as you have no more use to me."
    pc "Я отпущу тебя. Но только потому, что ты мне больше не нужна."

# game/script.rpy:957
translate rus cum_throat_a60bb5ce:

    # "With a powerful kick, I topple the girl to the floor, leaving her lying there gulping air with her mouth like a beached fish, cum dripping from the corner of her mouth."
    "Мощным ударом ноги я опрокидываю девушку на пол, и она лежит, глотая воздух ртом, как рыба на мели, а из уголка ее рта капает сперма."

# game/script.rpy:958
translate rus cum_throat_972897ae:

    # "Standing over her, watching as she gasps for air and tries to regain her composure, I smirk."
    "Стоя над ней и наблюдая за тем, как она задыхается и пытается вернуть себе самообладание, я ухмыляюсь."

# game/script.rpy:959
translate rus cum_throat_47da35ed:

    # "Knowing that I's left my mark on her both physically and emotionally is a realy powerfull stuff."
    "Осознание того, что я оставил на ней свой след, как физический, так и эмоциональный, - это очень мощная штука."

# game/script.rpy:960
translate rus cum_throat_b2f911fa:

    # pc "Remember this night."
    pc "Запомни эту ночь, Кристина."

# game/script.rpy:961
translate rus cum_throat_2609437f:

    # "There was my final words, before turning to leave the apartment."
    "Это были мои последние слова перед тем, как повернуться и выйти из квартиры."

# game/script.rpy:969
translate rus impregnate_42852fc9:

    # "I smirked down at her, enjoying her humiliation."
    "Я с ухмылкой смотрел на нее, наслаждаясь ее унижением."

# game/script.rpy:970
translate rus impregnate_353fe81f:

    # pc "On your hands and knees!"
    pc "Вставай раком, блядина!"

# game/script.rpy:971
translate rus impregnate_2b0950fa:

    # "I ordered once more, and Kristen complied, her body shaking with fear and exhaustion."
    "Я приказал еще раз, и Кристина подчинилась, ее тело тряслось от страха и изнеможения."

# game/script.rpy:972
translate rus impregnate_16fe4ce2:

    # "I moved behind her, positioning himself at her entrance. With one swift thrust, I entered her, causing her to cry out in pain as I stretched her beyond what she thought possible."
    "Я переместился к ней сзади, расположившись у ее входа. Одним быстрым толчком я вошел в нее, заставив ее вскрикнуть от боли, когда мой толстый хуй растянул ее до предела."

# game/script.rpy:973
translate rus impregnate_bf897eaa:

    # "I didn't give her a moment to adjust, instead pounding into her relentlessly, each thrust driving her further into the floor."
    "Я не давал ей ни секунды на адаптацию, вместо этого без устали вбиваясь в нее, с каждым толчком все глубже вжимая ее в твёрдый пол."

# game/script.rpy:974
translate rus impregnate_3191b453:

    # "Kristen could feel her tears falling onto the hardwood beneath her, mingling with the sweat that dripped from her body."
    "Кристина чувствовала, как ее слезы падают на твердый пол под ней, смешиваясь с потом, который капал с наших разгоряченных тел."

# game/script.rpy:975
translate rus impregnate_633816c7:

    # "As I fucked her brutally, I reached around and began to roughly massage her clit, my fingers digging into her sensitive flesh."
    "Жестоко трахая ее, я потянулся и начал грубо массировать ее клитор, впиваясь пальцами в чувствительную плоть."

# game/script.rpy:976
translate rus impregnate_95b6cc67:

    # k "No... please..."
    k "Нет... пожалуйста..."

# game/script.rpy:977
translate rus impregnate_a1161d39:

    # "She whimpered again, but her body betrayed her, responding to my touch despite the pain and humiliation."
    "Она снова захныкала, но ее тело предало ее, отвечая на мои прикосновения, несмотря на боль и унижение."

# game/script.rpy:978
translate rus impregnate_1f3c4f01:

    # "Her hips began to move in sync with my, her pussy clenching around my cock as she unwillingly approached another orgasm."
    "Ее бедра начали двигаться синхронно с моими, ее киска сжималась вокруг моего члена, когда она невольно приближалась к очередному оргазму."

# game/script.rpy:979
translate rus impregnate_b0b5eff7:

    # pc "You like that, bitch?"
    pc "Тебе это нравится, шлюха?"

# game/script.rpy:980
translate rus impregnate_9bb9228d:

    # "I growled, my breath hot on her neck."
    "Я зарычал и мое дыхание обжигает ее шею."

# game/script.rpy:981
translate rus impregnate_1df2ad9b:

    # pc "Do you like being fucked like a dirty little whore?"
    pc "Тебе нравится, когда тебя трахают, как маленькую грязную шлюху?"

# game/script.rpy:982
translate rus impregnate_71709e44:

    # "Kristen couldn't deny it - the pleasure was overwhelming, even amidst the fear and degradation."
    "Кристина не могла отрицать этого - удовольствие было непреодолимым, даже на фоне страха и унижения."

# game/script.rpy:983
translate rus impregnate_c7223c3e:

    # "She moaned loudly as her climax approached, her entire body trembling with the force of it."
    "Она громко застонала, когда наступила кульминация, и все ее тело задрожало от напряжения."

# game/script.rpy:984
translate rus impregnate_32bb6084:

    # "With a final thrust, I drove himself deep inside her, holding still as I came, my hot seed spilling into her."
    "С последним толчком я вошёл в нее, удерживая хуй глубоко внутри, и мое горячее семя хлынуло в нее."

# game/script.rpy:985
translate rus impregnate_3f5c6302:

    # "Kristen cried out, her own orgasm ripping through her as she shuddered beneath me. As the waves of pleasure subsided, she collapsed onto the floor, her body spent and her spirit broken."
    "Кристен закричала, ее собственный оргазм прорвался через нее, и она задрожала подо мной. Когда волны наслаждения стихли, она рухнула на пол, ее тело было истощено, а дух сломлен."

# game/script.rpy:986
translate rus impregnate_9a0f0bd2:

    # "I stood above her, my chest heaving from exertion. I looked down at her with disdain, as if she were nothing more than a used condom."
    "Я стоял над ней, моя грудь вздымалась от напряжения. Я смотрел на нее с презрением, как будто она была не более чем использованным презервативом."

# game/script.rpy:987
translate rus impregnate_e70c6fa8:

    # pc "I'm done with you."
    pc "Я закончил с тобой."

# game/script.rpy:995
translate rus cum_ass_bbbe5520:

    # "As I invide her rectum, Kristen screams again, her body shaking violently under the onslaught of my thrusts."
    "Когда я вторгаюсь в ее прямую кишку, Кристен снова кричит, ее тело неистово содрогается под натиском моего толстого хуя."

# game/script.rpy:996
translate rus cum_ass_46023fbf:

    # "I can feel her tightness starting to give way, her ass adjusting slightly to accommodate my girth."
    "Я чувствую, как она начинает поддаваться, как ее попка слегка подстраивается под мои размеры, растягиваясь всё больше."

# game/script.rpy:997
translate rus cum_ass_f7d92360:

    # pc "That's it, take my cock like a good little whore."
    pc "Вот так, принимай мой хуй в свою жопу, как хорошая анальная рабыня."

# game/script.rpy:998
translate rus cum_ass_11a00085:

    # "Kristen's vision goes blurry as I continues to pound into her, the pain becoming too much for her to handle."
    "Зрение Кристен затуманивается, когда я продолжаю входить в нее, и боль становится слишком сильной, чтобы она могла с ней справиться."

# game/script.rpy:999
translate rus cum_ass_a8cf6f06:

    # "She feels like she's going to black out from the intensity of it all, but I show no signs of stopping."
    "Ей кажется, что она вот-вот потеряет сознание от интенсивности происходящего, но я не подаю признаков остановки."

# game/script.rpy:1000
translate rus cum_ass_3eeb3423:

    # "I grunt, feeling my orgasm building deep within me. I grabs Kristen's hair roughly, pulling her head back as I thrusts even harder into her ass."
    "Я издаю животные звуки, чувствуя, как оргазм нарастает внутри меня. Грубо хватаю Кристину за волосы, оттягивая ее голову назад, еще сильнее долбя её уже полностью растнутую задницу."

# game/script.rpy:1001
translate rus cum_ass_8fbc746b:

    # pc "You're gonna take my cum."
    pc "Принимай мою сперму."

# game/script.rpy:1002
translate rus cum_ass_53fa103a:

    # "My voice rough with lust and dominance."
    "Мой голос груб от вожделения и доминирования."

# game/script.rpy:1003
translate rus cum_ass_da550b34:

    # pc "And you'll fucking love it!"
    pc "И тебе это, блядь, понравится!"

# game/script.rpy:1004
translate rus cum_ass_dee4fa18:

    # "Kristen whimpers, feeling my grip on her hair tightening as I continues to pound into her."
    "Кристина хнычет, чувствуя, как крепнет моя хватка на ее волосах, пока я продолжаю долбить её."

# game/script.rpy:1005
translate rus cum_ass_d7a7e399:

    # "She can feel my cock throbbing inside her, ready to explode at any moment."
    "Она чувствует, как мой член пульсирует внутри, готовый взорваться в любой момент."

# game/script.rpy:1006
translate rus cum_ass_4a1afae4:

    # k "Please… stop..."
    k "Умоляю... хватит..."

# game/script.rpy:1007
translate rus cum_ass_9218ef9b:

    # "She begs weakly, her body shaking with each brutal stroke."
    "Она слабо умоляет, ее тело содрогается от каждого жестокого удара."

# game/script.rpy:1008
translate rus cum_ass_a7ae65e9:

    # "My orgasm crashes over me like a wave, and I grunts loudly as I empties me balls deep inside her bowels."
    "Оргазм накатывает на меня, как волна, и я громко хриплю, опустошая свои яйца глубоко в ее прямую кишку."

# game/script.rpy:1009
translate rus cum_ass_f145e642:

    # "Kristen screams in pain and pleasure as she feels my hot seed filling her up, the intensity of it all too much to bear."
    "Кристен кричит от боли и удовольствия, чувствуя, как мое горячее семя заполняет ее, и интенсивность этого слишком велика, чтобы выдержать."

# game/script.rpy:1010
translate rus cum_ass_0a4e3461:

    # "I pull out slowly, leaving her gasping for air on the floor."
    "Я медленно выхожу из нее, оставляя ее задыхающейся на полу."

# game/script.rpy:1011
translate rus cum_ass_10e81e48:

    # pc "You took it like a good little slut."
    pc "Вот так чувствую себя настоящие подстилки."

# game/script.rpy:1012
translate rus cum_ass_0f1fc052:

    # "I pant, wiping my sweaty brow with the back of my hand."
    "Я задыхаюсь, вытирая потный лоб тыльной стороной ладони."

# game/script.rpy:1013
translate rus cum_ass_98a3f194:

    # "Kristen lies on the floor, her body shaking and covered in sweat. She feels drained, both physically and emotionally, as she tries to process what just happened."
    "Кристина лежит на полу, ее тело трясется и покрыто потом. Она чувствует себя истощенной как физически, так и эмоционально, пытаясь осмыслить произошедшее."

# game/script.rpy:1014
translate rus cum_ass_37398811:

    # "Tears stream down her face as she looks up at I through tear-filled eyes."
    "Слезы текут по ее лицу, когда она смотрит на меня глазами, полными слез."

# game/script.rpy:1015
translate rus cum_ass_756fd4c3:

    # k "Why…?"
    k "За что?..."

# game/script.rpy:1016
translate rus cum_ass_b05d1d41:

    # "She whispers weakly, not understanding why I did this to her."
    "Она слабо шепчет, не понимая, почему я так с ней поступил."

# game/script.rpy:1017
translate rus cum_ass_cabe23c2:

    # "I smirk, looking down at her with a mix of satisfaction and contempt."
    "Я ухмыляюсь, глядя на нее со смесью удовлетворения и презрения."

# game/script.rpy:1018
translate rus cum_ass_475a6517:

    # pc "Because I could."
    pc "Потому что я могу."

# game/script.rpy:1019
translate rus cum_ass_a6e655c8:

    # "I say it felling empowered by this show of domination, before turning to leave the room."
    "Я говорю это, чувствуя, что обретаю силу от этого демонстративного доминирования, и поворачиваюсь, чтобы выйти из комнаты."

# game/script.rpy:1027
translate rus flee_9535993e:

    # "She lying motionless on a floor in a puddle of my semen. Time to get out!"
    "Она неподвижно лежит на полу в луже моей спермы. Пора выходить!"

# game/script.rpy:1029
translate rus flee_0c052b1f:

    # "FIN"
    "FIN"

translate rus strings:

    # game/script.rpy:325
    old "Make love to her"
    new "Зняться любовью"

    # game/script.rpy:325
    old "Rape her brutaly"
    new "Изнасиловать её жестоко"

    # game/script.rpy:401
    old "Blowjob"
    new "Минет"

    # game/script.rpy:401
    old "Sex"
    new "Секс"

    # game/script.rpy:401
    old "Anal"
    new "Анальный"

    # game/script.rpy:751
    old "Fuck her throat"
    new "Выебать в горло"

    # game/script.rpy:751
    old "Forced sex"
    new "Изнасиловать"

    # game/script.rpy:751
    old "Anal rape"
    new "Содомировать"

